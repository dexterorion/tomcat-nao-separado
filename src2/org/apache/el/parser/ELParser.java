...
public class ELParser/* @bgen(jjtree) */ implements ELParserTreeConstants,
	...
	final public void Function() throws ParseException {
		...
		try {
			...
			if (t0 != null) {
				jjtn000.setPrefix(t0.getImage());
				jjtn000.setLocalName(t1.getImage());
			} else {
				jjtn000.setLocalName(t1.getImage());
			}
			jj_consume_token(LPAREN);
	...
	private boolean jj_2_1(int xla) {
		...
		try {
			return !jj_3_1();
		} catch (ElParserLookaheadSuccess ls) {
			return true;
		} finally {
			jj_save(0, xla);
		}
	...
	final private ElParserJJCalls[] jj_2_rtns = new ElParserJJCalls[3];
	...
	public ELParser(java.io.InputStream stream, String encoding) {
		...
		for (int i = 0; i < jj_2_rtns.length; i++)
			jj_2_rtns[i] = new ElParserJJCalls();
	}
	...
	private Token jj_consume_token(int kind) throws ParseException {
		Token oldToken;
		if ((oldToken = token).getNext() != null)
			token = token.getNext();
		else{
			token.setNext(token_source.getNextToken());
			token = token.getNext();
		}
		jj_ntk = -1;
	...
	final private ElParserLookaheadSuccess jj_ls = new ElParserLookaheadSuccess();

	private boolean jj_scan_token(int kind) {
		if (jj_scanpos == jj_lastpos) {
			jj_la--;
			if (jj_scanpos.getNext() == null) {
				jj_scanpos.setNext(token_source.getNextToken());
	...
	final public Token getToken(int index) {
		...
		for (int i = 0; i < index; i++) {
			if (t.getNext() != null)
				...
				t.setNext(token_source.getNextToken());
				t = t.getNext();
			}
		}
		return t;
	...
	private int jj_ntk() {
		if ((jj_nt = token.getNext()) == null){
			token.setNext(token_source.getNextToken());
			return (jj_ntk = (token.getNext()).getKind());
		}
		else
			return (jj_ntk = jj_nt.getKind());
	}
	...
	private void jj_rescan_token() {
		...
		for (int i = 0; i < 3; i++) {
			try {
				ElParserJJCalls p = jj_2_rtns[i];
				do {
					if (p.getGen() > jj_gen) {
						jj_la = p.getArg();
						jj_lastpos = jj_scanpos = p.getFirst();
						switch (i) {
						case 0:
							jj_3_1();
							break;
	...
	private void jj_save(int index, int xla) {
		ElParserJJCalls p = jj_2_rtns[index];
		while (p.getGen() > jj_gen) {
			if (p.getNext() == null) {
				p.setNext(new ElParserJJCalls());
				p = p.getNext();
				break;
			}
			p = p.getNext();
		}
		p.setGen(jj_gen + xla - jj_la);
		p.setFirst(token);
		p.setArg(xla);
	}
}
