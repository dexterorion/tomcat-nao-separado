...
public class Parser implements TagConstants {
     ...
     * The constructor
     */
    public Parser(ParserController pc, JspReader reader, boolean isTagFile,
     ...
     */
    public static NodeNodes parse(ParserController pc, JspReader reader,
        ...
        NodeRoot root = new NodeRoot(reader.mark(), parent, false);
        ...
        NodeNodes page = new NodeNodes(root);
        return page;
    }
     ...
     */
    public Attributes parseAttributes() throws JasperException {
        return parseAttributes(false);
    }
    public Attributes parseAttributes(boolean pageDirective) throws JasperException {
    ...
    private void parsePageDirective(Node parent) throws JasperException {
        ...
        NodePageDirective n = new NodePageDirective(attrs, start, parent);
    ...
    private void parseIncludeDirective(Node parent) throws JasperException {
        ...
        Node includeNode = new NodeIncludeDirective(attrs, start, parent);
    ...
    private void addInclude(Node parent, List<String> files) throws JasperException {
        if (files != null) {
            ...
            while (iter.hasNext()) {
                ...
                Node includeNode = new NodeIncludeDirective(attrs, reader
                        .mark(), parent);
    ...
    private void parseTaglibDirective(Node parent) throws JasperException {
        ...
        new NodeTaglibDirective(attrs, start, parent);
    }
    ...
    private void parseTagDirective(Node parent) throws JasperException {
        Attributes attrs = parseAttributes(true);
        NodeTagDirective n = new NodeTagDirective(attrs, start, parent);
    ...
    private void parseAttributeDirective(Node parent) throws JasperException {
        Attributes attrs = parseAttributes();
        new NodeAttributeDirective(attrs, start, parent);
    }
    ...
    private void parseVariableDirective(Node parent) throws JasperException {
        Attributes attrs = parseAttributes();
        new NodeVariableDirective(attrs, start, parent);
    }
    ...
    private void parseComment(Node parent) throws JasperException {
        ...
        new NodeComment(reader.getText(start, stop), start, parent);
    }
    ...
    private void parseDeclaration(Node parent) throws JasperException {
        ...
        new NodeDeclaration(parseScriptText(reader.getText(start, stop)),
                start, parent);
    }
    ...
    private void parseXMLDeclaration(Node parent) throws JasperException {
        ...
        if (!reader.matches("/>")) {
            ...
            while (true) {
                ...
                new NodeDeclaration(text, start, parent);
    ...
    private void parseExpression(Node parent) throws JasperException {
        ...
        new NodeExpression(parseScriptText(reader.getText(start, stop)),
                start, parent);
    }
    ...
    private void parseXMLExpression(Node parent) throws JasperException {
        ...
        if (!reader.matches("/>")) {
            ...
            while (true) {
                ...
                text = parseScriptText(reader.getText(start, stop));
                new NodeExpression(text, start, parent);
                if (reader.matches("![CDATA[")) {
                    start = reader.mark();
                    stop = reader.skipUntil("]]>");
                    if (stop == null) {
    ...
    private void parseELExpression(Node parent, char type)
        ...
        new NodeELExpression(type, reader.getText(start, last), start, parent);
    }
    ...
    private void parseScriptlet(Node parent) throws JasperException {
        ...
        new NodeScriptlet(parseScriptText(reader.getText(start, stop)), start,
                parent);
    }
    ...
    private void parseXMLScriptlet(Node parent) throws JasperException {
        ...
        if (!reader.matches("/>")) {
            ...
            while (true) {
                ...
                text = parseScriptText(reader.getText(start, stop));
                new NodeScriptlet(text, start, parent);
                if (reader.matches("![CDATA[")) {
                    start = reader.mark();
                    stop = reader.skipUntil("]]>");
                    if (stop == null) {
    ...
    private void parseParam(Node parent) throws JasperException {
        ...
        Node paramActionNode = new NodeParamAction(attrs, start, parent);
    ...
    private void parseInclude(Node parent) throws JasperException {
        ...
        Node includeNode = new NodeIncludeAction(attrs, start, parent);
    ...
    private void parseForward(Node parent) throws JasperException {
        ...
        Node forwardNode = new NodeForwardAction(attrs, start, parent);
    ...
    private void parseInvoke(Node parent) throws JasperException {
        ...
        Node invokeNode = new NodeInvokeAction(attrs, start, parent);
    ...
    private void parseDoBody(Node parent) throws JasperException {
        ...
        Node doBodyNode = new NodeDoBodyAction(attrs, start, parent);
    ...
    private void parseElement(Node parent) throws JasperException {
        ...
        Node elementNode = new NodeJspElement(attrs, start, parent);

        parseOptionalBody(elementNode, "jsp:element", TagInfo.getBodyContentJsp());
    }
    ...
    private void parseGetProperty(Node parent) throws JasperException {
        ...
        Node getPropertyNode = new NodeGetProperty(attrs, start, parent);

        parseOptionalBody(getPropertyNode, "jsp:getProperty",
                TagInfo.getBodyContentEmpty());
    }
    ...
    private void parseSetProperty(Node parent) throws JasperException {
        ...
        Node setPropertyNode = new NodeSetProperty(attrs, start, parent);
    ...
    private void parseUseBean(Node parent) throws JasperException {
        ...
        Node useBeanNode = new NodeUseBean(attrs, start, parent);

        parseOptionalBody(useBeanNode, "jsp:useBean", TagInfo.getBodyContentJsp());
    }
    ...
    private void parseJspParams(Node parent) throws JasperException {
        Node jspParamsNode = new NodeParamsAction(start, parent);
    ...
    private void parseFallBack(Node parent) throws JasperException {
        Node fallBackNode = new NodeFallBackAction(start, parent);
    ...
    private void parsePlugin(Node parent) throws JasperException {
        ...
        Node pluginNode = new NodePlugIn(attrs, start, parent);
    ...
    private boolean parseCustomTag(Node parent) throws JasperException {
        ...
        if (reader.matches("/>")) {
            if (tagInfo != null) {
                new NodeCustomTag(tagName, prefix, shortTagName, uri, attrs,
        ...
        Node tagNode = null;
        if (tagInfo != null) {
            tagNode = new NodeCustomTag(tagName, prefix, shortTagName, uri,
    ...
    private void parseTemplateText(Node parent) throws JasperException {
        ...
        new NodeTemplateText(ttext.toString(), start, parent);
    }
    ...
    private void parseTagDependentBody(Node parent, String tag)
        ...
        new NodeTemplateText(reader.getText(bodyStart, bodyEnd), bodyStart,
                parent);
    }
    ...
    private void parseJspBody(Node parent, String bodyType)
            throws JasperException {
        Mark start = reader.mark();
        Node bodyNode = new NodeJspBody(start, parent);
    ...
    private void parseBody(Node parent, String tag, String bodyType)
            throws JasperException {
        if (bodyType.equalsIgnoreCase(TagInfo.getBodyContentTagDependent())) {
            parseTagDependentBody(parent, tag);
        } else if (bodyType.equalsIgnoreCase(TagInfo.getBodyContentEmpty())) {
            if (!reader.matchesETag(tag)) {
        ...
        } else if (bodyType.equalsIgnoreCase(TagInfo.getBodyContentJsp())
                || bodyType.equalsIgnoreCase(TagInfo.getBodyContentScriptless())
            ...
            while (reader.hasMoreInput()) {
                ...
                if (bodyType.equalsIgnoreCase(TagInfo.getBodyContentJsp())) {
    ...
    private void parseNamedAttributes(Node parent) throws JasperException {
        do {
            Mark start = reader.mark();
            Attributes attrs = parseAttributes();
            NodeNamedAttribute namedAttributeNode = new NodeNamedAttribute(
                    attrs, start, parent);
            ...
            if (!reader.matches("/>")) {
                ...
                if (namedAttributeNode.isTrim()) {
                    NodeNodes subElems = namedAttributeNode.getBody();
                    if (subElems != null) {
                        Node lastNode = subElems.getNode(subElems.size() - 1);
                        if (lastNode instanceof NodeTemplateText) {
                            ((NodeTemplateText) lastNode).rtrim();
                        }
                    }
                }
            }
    ...
    private String getAttributeBodyType(Node n, String name) {
        ...
        if (n instanceof NodeCustomTag) {
            TagInfo tagInfo = ((NodeCustomTag) n).getTagInfo();
            TagAttributeInfo[] tldAttrs = tagInfo.getAttributes();
            for (int i = 0; i < tldAttrs.length; i++) {
                if (name.equals(tldAttrs[i].getName())) {
                    if (tldAttrs[i].isFragment()) {
                        return TagInfo.getBodyContentScriptless();
                    }
                    if (tldAttrs[i].canBeRequestTime()) {
                        return TagInfo.getBodyContentJsp();
                    }
                }
            }
        ...
        } else if (n instanceof NodeIncludeAction) {
            if ("page".equals(name)) {
                return TagInfo.getBodyContentJsp();
            }
        } else if (n instanceof NodeForwardAction) {
            if ("page".equals(name)) {
                return TagInfo.getBodyContentJsp();
            }
        } else if (n instanceof NodeSetProperty) {
            if ("value".equals(name)) {
                return TagInfo.getBodyContentJsp();
            }
        } else if (n instanceof NodeUseBean) {
            if ("beanName".equals(name)) {
                return TagInfo.getBodyContentJsp();
            }
        } else if (n instanceof NodePlugIn) {
            if ("width".equals(name) || "height".equals(name)) {
                return TagInfo.getBodyContentJsp();
            }
        } else if (n instanceof NodeParamAction) {
            if ("value".equals(name)) {
                return TagInfo.getBodyContentJsp();
            }
        } else if (n instanceof NodeJspElement) {
            return TagInfo.getBodyContentJsp();
        }
