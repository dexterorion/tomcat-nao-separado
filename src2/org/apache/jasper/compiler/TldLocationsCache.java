...
public class TldLocationsCache {
	...
	static {
		...
		StringBuilder jarList = new StringBuilder(System.getProperty(
				Constants28.getDefaultJarSkipProp(), ""));

		String tldJars = System.getProperty(Constants28.getTldJarSkipProp(), "");
		if (tldJars.length() > 0) {
			if (jarList.length() > 0) {
				jarList.append(',');
			}
		...
		try {
			...
			if (jarScanner != null) {
				jarScanner.scan(ctxt, Thread.currentThread()
						...
						new TldLocationsCacheTldJarScannerCallback(this),
						noTldJars);
			}
	...
	private void tldScanWebXml() throws Exception {
		...
		try {
			...
			boolean validate = Boolean.parseBoolean(ctxt
					.getInitParameter(Constants28.getXmlValidationInitParam()));
			String blockExternalString = ctxt.getInitParameter(Constants28
					.getXmlBlockExternalInitParam());
	 ...
	 */
	public void tldScanDir(File start) throws IOException {
	 ...
	 * Keep in sync with o.a.c.startup.TldConfig
	 */
	public void tldScanJar(JarURLConnection jarConn) throws IOException {
	...
	private void tldScanStream(String resourcePath, String entryName,
		...
		try {
			...
			boolean validate = Boolean
					.parseBoolean(ctxt.getInitParameter(Constants28
							.getXmlValidationTldInitParam()));
			String blockExternalString = ctxt.getInitParameter(Constants28
					.getXmlBlockExternalInitParam());
			boolean blockExternal;
			if (blockExternalString == null) {
	...
	public static Set<String> getNoTldJars() {
		return noTldJars;
	}

	public static void setNoTldJars(Set<String> noTldJars) {
		TldLocationsCache.noTldJars = noTldJars;
	}

	public static boolean isShowTldScanWarning() {
		return showTldScanWarning;
	}

	public static void setShowTldScanWarning(boolean showTldScanWarning) {
		TldLocationsCache.showTldScanWarning = showTldScanWarning;
	}

	public Hashtable<String, TldLocation> getMappings() {
		return mappings;
	}

	public void setMappings(Hashtable<String, TldLocation> mappings) {
	...
	public ServletContext getCtxt() {
		return ctxt;
	}

	public void setCtxt(ServletContext ctxt) {
		this.ctxt = ctxt;
	}
	...
	public static int getAbsUri() {
		return ABS_URI;
	}

	public static int getRootRelUri() {
		return ROOT_REL_URI;
	}

	public static int getNorootRelUri() {
		return NOROOT_REL_URI;
	}
	...
	public static String getTldExt() {
		return TLD_EXT;
	}
