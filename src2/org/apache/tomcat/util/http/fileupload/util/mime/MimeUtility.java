...
public final class MimeUtility {
     ...
     * @throws ParseException2
     * @throws UnsupportedEncodingException
     */
    private static String decodeWord(String word) throws ParseException2, UnsupportedEncodingException {
        // encoded words start with the characters "=?".  If this not an encoded word, we throw a
        // ParseException for the caller.

        if (!word.startsWith(ENCODED_TOKEN_MARKER)) {
            throw new ParseException2("Invalid RFC 2047 encoded-word: " + word);
        }

        int charsetPos = word.indexOf('?', 2);
        if (charsetPos == -1) {
            throw new ParseException2("Missing charset in RFC 2047 encoded-word: " + word);
        }
        ...
        if (encodingPos == -1) {
            throw new ParseException2("Missing encoding in RFC 2047 encoded-word: " + word);
        }
        ...
        if (encodedTextPos == -1) {
            throw new ParseException2("Missing encoded text in RFC 2047 encoded-word: " + word);
        }
        ...
        try {
            ...
            if (encoding.equals(BASE64_ENCODING_MARKER)) {
                decodedData = Base642.decodeBase64(encodedText);
