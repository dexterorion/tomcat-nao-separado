...
public class SetPropertyRule extends Rule {
    ...
    public void begin(String namespace, String theName, Attributes attributes)
        ...
        if (getDigester().getLog().isDebugEnabled()) {
            getDigester().getLog().debug("[SetPropertyRule]{" + getDigester().getMatch() +
                    "} Set " + top.getClass().getName() + " property " +
                    actualName + " to " + actualValue);
        }

        // Set the property (with conversion as necessary)
        if (!getDigester().isFakeAttribute(top, actualName) 
                && !IntrospectionUtils.setProperty(top, actualName, actualValue) 
                && getDigester().getRulesValidation()) {
            getDigester().getLog().warn("[SetPropertyRule]{" + getDigester().getMatch() +
