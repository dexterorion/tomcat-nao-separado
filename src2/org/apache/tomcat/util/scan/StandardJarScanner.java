...
public class StandardJarScanner implements JarScanner {
    ...
    private static final StringManager3 sm =
        StringManager3.getManager(Constants35.getPackage());

    static {
        String jarList = System.getProperty(Constants35.getSkipJarsProperty());
        if (jarList != null) {
     ...
     *                      defined by {@link Constants35#SKIP_JARS_PROPERTY}
     */
    @Override
    public void scan(ServletContext context, ClassLoader classloader,
        ...
        // Scan WEB-INF/lib
        Set<String> dirList = context.getResourcePaths(Constants35.getWebInfLib());
        if (dirList != null) {
            Iterator<String> it = dirList.iterator();
            while (it.hasNext()) {
                String path = it.next();
                if (path.endsWith(Constants35.getJarExt()) &&
                        ...
                        if (jarName != null &&
                            !(Matcher.matchName(ignoredJars, jarName) ||
                                urls[i].toString().contains(
                                        Constants35.getWebInfLib() + jarName))) {
                            if (log.isDebugEnabled()) {
    ...
    private void process(JarScannerCallback callback, URL url)
        ...
        if (conn instanceof JarURLConnection) {
            ...
            if (urlStr.startsWith("file:") || urlStr.startsWith("jndi:") ||
                ...
                if (urlStr.endsWith(Constants35.getJarExt())) {
    ...
    private String getJarName(URL url) {
        ...
        int end = path.indexOf(Constants35.getJarExt());
        if (end != -1) {
