...
public class AprServletInputStream extends AbstractServletInputStream {
	...
	protected int doRead(boolean block, byte[] b, int off, int len)
		...
		if (closed) {
			throw new IOException(getSm().getString("apr.closed",
					Long.valueOf(socket)));
		}
		...
		if (!readDone) {
			try {
				...
				Socket.optSet(socket, Socket.getAprSoNonblock(),
						(block ? 0 : 1));
				// Downgrade the lock
				try {
					readLock.lock();
		...
		if (result > 0) {
			eagain = false;
			return result;
		} else if (-result == Status.getEagain()) {
			eagain = true;
			return 0;
		} else if (-result == Status.getAprEgeneral() && wrapper.isSecure()) {
			...
			if (log.isDebugEnabled()) {
				log.debug(getSm().getString("apr.read.sslGeneralError",
						Long.valueOf(socket), wrapper));
			}
			eagain = true;
			return 0;
		} else if (-result == Status.getAprEof()) {
			throw new EOFException(getSm().getString("apr.clientAbort"));
		} else if ((OS.isWin32() || OS.isWin64())
				&& (-result == Status.getAprOsStartSyserr() + 10053)) {
			// 10053 on Windows is connection aborted
			throw new EOFException(getSm().getString("apr.clientAbort"));
		} else {
			throw new IOException(getSm().getString("apr.read.error",
