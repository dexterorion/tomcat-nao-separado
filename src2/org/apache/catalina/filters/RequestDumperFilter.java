...
public class RequestDumperFilter implements Filter {
    ...
    private static final ThreadLocal<RequestDumperFilterTimestamp> timestamp = new ThreadLocal6();
    ...
    private String getTimestamp() {
    	RequestDumperFilterTimestamp ts = timestamp.get();
        long currentTime = System.currentTimeMillis();
        
        if ((ts.getDate().getTime() + 999) < currentTime) {
            ts.getDate().setTime(currentTime - (currentTime % 1000));
            ts.update();
        }
        return ts.getDateString();
    }
