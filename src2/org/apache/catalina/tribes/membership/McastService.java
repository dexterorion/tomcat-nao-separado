...
public class McastService implements MembershipService,MembershipListener,MessageListener {
     ...
     */
    private static final StringManager sm = StringManager.getManager(Constants19.getPackage());
    ...
    public boolean hasMembers() {
       if ( impl == null || impl.getMembership() == null ) return false;
       return impl.getMembership().hasMembers();
    }

    @Override
    public Member getMember(Member mbr) {
        if ( impl == null || impl.getMembership() == null ) return null;
        return impl.getMembership().getMember(mbr);
    }
    ...
    public Member[] getMembers() {
        if ( impl == null || impl.getMembership() == null ) return EMPTY_MEMBERS;
        return impl.getMembership().getMembers();
    }
    /**
    ...
    public void broadcast(ChannelMessage message) throws ChannelException {
        if (impl==null || (impl.getStartLevel() & Channel.MBR_TX_SEQ)!=Channel.MBR_TX_SEQ )
        ...
        if (data.length>McastServiceImpl.getMAX_PACKET_SIZE()) {
            throw new ChannelException("Packet length["+data.length+"] exceeds max packet size of "+McastServiceImpl.getMAX_PACKET_SIZE()+" bytes.");
        }
