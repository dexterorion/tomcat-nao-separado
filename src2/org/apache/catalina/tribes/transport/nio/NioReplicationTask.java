...
import org.apache.catalina.tribes.transport.Constants20;
...
public class NioReplicationTask extends AbstractRxTask {
    ...
    public synchronized void run() {
        if ( buffer == null ) {
            ...
            if ( (getOptions() & getOptionDirectBuffer()) == getOptionDirectBuffer()) {
    ...
    protected void drainChannel (final SelectionKey key, ObjectReader reader) throws Exception {
        ...
        ChannelMessage[] msgs = pkgcnt == 0? ChannelData.getEmptyDataArray() : reader.execute();
        ...
        for ( int i=0; i<msgs.length; i++ ) {
             ...
             */
            if (ChannelData.sendAckAsync(msgs[i].getOptions())) sendAck(key,(WritableByteChannel)channel,Constants20.getAckCommand(),saddr);
            try {
                if ( Logs.getMessages().isTraceEnabled() ) {
                    try {
                        Logs.getMessages().trace("NioReplicationThread - Received msg:" + new UniqueId(msgs[i].getUniqueId()) + " at " + new java.sql.Timestamp(System.currentTimeMillis()));
                    }catch ( Throwable t ) {}
                }
                //process the message
                 ...
                 */
                if (ChannelData.sendAckSync(msgs[i].getOptions())) sendAck(key,(WritableByteChannel)channel,Constants20.getAckCommand(),saddr);
            }catch ( RemoteProcessException e ) {
                if ( log.isDebugEnabled() ) log.error("Processing of cluster message failed.",e);
                if (ChannelData.sendAckSync(msgs[i].getOptions())) sendAck(key,(WritableByteChannel)channel,Constants20.getFailAckCommand(),saddr);
            }catch ( Exception e ) {
