...
public class BioSender extends AbstractSender {
     ...
     */
    private static final StringManager sm = StringManager.getManager(Constants20.getPackage());
    ...
    private XByteBuffer ackbuf = new XByteBuffer(Constants20.getAckCommand().length,true);
    ...
    protected void waitForAck() throws java.io.IOException {
        try {
            ...
            int bytesRead = 0;
            int i = soIn.read();
            while ((i != -1) && (bytesRead < Constants20.getAckCommand().length)) {
                bytesRead++;
                byte d = (byte)i;
                ackbuf.append(d);
                if (ackbuf.doesPackageExist() ) {
                    byte[] ackcmd = ackbuf.extractDataPackage(true).getBytes();
                    ackReceived = Arrays.equals(ackcmd,Constants20.getAckData());
                    failAckReceived = Arrays.equals(ackcmd,Constants20.getFailAckData());
