...
public class OrderInterceptor extends ChannelInterceptorBase {
    private HashMap<Member, OrderInterceptorCounter> outcounter = new HashMap<Member, OrderInterceptorCounter>();
    private HashMap<Member, OrderInterceptorCounter> incounter = new HashMap<Member, OrderInterceptorCounter>();
    private HashMap<Member, OrderInterceptorMessageOrder> incoming = new HashMap<Member, OrderInterceptorMessageOrder>();
    private long expire = 3000;
    private boolean forwardExpired = true;
    private int maxQueue = Integer.MAX_VALUE;
    
    private final ReentrantReadWriteLock inLock = new ReentrantReadWriteLock(true);
    private final ReentrantReadWriteLock outLock= new ReentrantReadWriteLock(true);
    ...
    public void messageReceived(ChannelMessage msg) {
        ...
        msg.getMessage().trim(4);
        OrderInterceptorMessageOrder order = new OrderInterceptorMessageOrder(msgnr,(ChannelMessage)msg.deepclone());
        try {
    ...
    protected void processLeftOvers(Member member, boolean force) {
    	OrderInterceptorMessageOrder tmp = incoming.get(member);
        if ( force ) {
        	OrderInterceptorCounter cnt = getInCounter(member);
     ...
     */
    protected boolean processIncoming(OrderInterceptorMessageOrder order) {
        ...
        OrderInterceptorMessageOrder tmp = incoming.get(member);
        if ( tmp != null ) {
            order = OrderInterceptorMessageOrder.add(tmp,order);
        }
        ...
        OrderInterceptorMessageOrder head = order;
    ...
    protected int incCounter(Member mbr) { 
    	OrderInterceptorCounter cnt = getOutCounter(mbr);
        return cnt.inc();
    }
    
    protected OrderInterceptorCounter getInCounter(Member mbr) {
    	OrderInterceptorCounter cnt = incounter.get(mbr);
        if ( cnt == null ) {
            cnt = new OrderInterceptorCounter();
    ...
    protected OrderInterceptorCounter getOutCounter(Member mbr) {
    	OrderInterceptorCounter cnt = outcounter.get(mbr);
        if ( cnt == null ) {
            cnt = new OrderInterceptorCounter();
