...
public class GroupChannel extends ChannelInterceptorBase implements ManagedChannel {
     ...
     */
    private long heartbeatSleeptime = 5*1000;//every 5 seconds

    /**
     * Internal heartbeat thread
     */
    private GroupChannelHeartbeatThread hbthread = null;
     ...
     */
    private List<Object> membershipListeners = new CopyOnWriteArrayList<Object>();

    /**
     * A list of channel listeners that subscribe to incoming messages
     */
    private List<Object> channelListeners = new CopyOnWriteArrayList<Object>();
    ...
    public UniqueId send(Member[] destination, Serializable msg, int options, ErrorHandler handler) throws ChannelException {
        ...
        try {
            ...
            if ( Logs.getMessages().isTraceEnabled() ) {
                Logs.getMessages().trace("GroupChannel - Sent msg:" + new UniqueId(data.getUniqueId()) + " at " +new java.sql.Timestamp(System.currentTimeMillis())+ " to "+Arrays.toNameString(destination));
                Logs.getMessages().trace("GroupChannel - Send Message:" + new UniqueId(data.getUniqueId()) + " is " +msg);
            }
    ...
    public void messageReceived(ChannelMessage msg) {
        ...
        try {
            if ( Logs.getMessages().isTraceEnabled() ) {
                Logs.getMessages().trace("GroupChannel - Received msg:" + new UniqueId(msg.getUniqueId()) + " at " +new java.sql.Timestamp(System.currentTimeMillis())+ " from "+msg.getAddress().getName());
            }
            ...
            if ( Logs.getMessages().isTraceEnabled() ) {
                Logs.getMessages().trace("GroupChannel - Receive Message:" + new UniqueId(msg.getUniqueId()) + " is " +fwd);
            }
            ...
            if ( Logs.getMessages().isTraceEnabled() ) {
                Logs.getMessages().trace("GroupChannel delivered["+delivered+"] id:"+new UniqueId(msg.getUniqueId()));
            }
    ...
    protected void sendNoRpcChannelReply(RpcMessage msg, Member destination) {
        try {
            //avoid circular loop
            if ( msg instanceof RcpMessageNoRpcChannelReply) return;
            RcpMessageNoRpcChannelReply reply = new RcpMessageNoRpcChannelReply(msg.getRpcId(),msg.getUuid());
    ...
    public synchronized void start(int svc) throws ChannelException {
        ...
        if ( hbthread == null && heartbeat ) {
            hbthread = new GroupChannelHeartbeatThread(this,heartbeatSleeptime);
            hbthread.start();
        }
    }
    ...
    public Iterator<ChannelInterceptor> getInterceptors() {
        return new GroupChannelInterceptorIterator(this.getNext(),this.coordinator);
    }
