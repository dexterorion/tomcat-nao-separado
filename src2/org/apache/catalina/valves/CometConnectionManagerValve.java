...
public class CometConnectionManagerValve extends ValveBase
     ...
     */
    private final List<Request> cometRequests =
     ...
     */
    private final String cometRequestsAttribute =
    ...
    public void lifecycleEvent(LifecycleEvent event) {
        if (Lifecycle.BEFORE_STOP_EVENT.equals(event.getType())) {
            ...
            while (iterator.hasNext()) {
                ...
                try {
                    cometEvent.setEventType(CometEventEventType.END);
                    cometEvent.setEventSubType(
                            CometEventEventSubType.WEBAPP_RELOAD);
                    getNext().event(request, request.getResponse(), cometEvent);
                } catch (Exception e) {
                    getContainer().getLogger().warn(
                            getSm().getString("cometConnectionManagerValve.event"),
                            e);
                } finally {
                    try {
                        cometEvent.close();
                    } catch (IOException e) {
                        getContainer().getLogger().warn(getSm().getString(
    ...
    public void event(Request request, Response response, CometEvent event)
        ...
        try {
            ...
            if (!ok || response.isClosed()
                    || (event.getEventType() == CometEventEventType.END)
                    || (event.getEventType() == CometEventEventType.ERROR
        ...
        if (reqs != null) {
            for (int i = 0; i < reqs.length; i++) {
                ...
                try {
                    ...
                    event.setEventType(CometEventEventType.END);
                    event.setEventSubType(CometEventEventSubType.SESSION_END);
                    ((CometProcessor)
                            req.getWrapper().getServlet()).event(event);
                    event.close();
                } catch (Exception e) {
                    req.getWrapper().getParent().getLogger().warn(getSm().getString(
