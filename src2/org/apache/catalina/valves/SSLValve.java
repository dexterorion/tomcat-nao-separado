    ...
    public void invoke(Request request, Response response)
        ...
        if (strcert0 != null && strcert0.length()>28) {
            ...
            try {
                ...
                log.warn(getSm().getString("sslValve.certError", strcerts), e);
            } catch (NoSuchProviderException e) {
                log.error(getSm().getString(
                        "sslValve.invalidProvider", providerName), e);
            }
            request.setAttribute(Globals.getCertificatesAttr(), jsseCerts);
        }
        strcert0 = mygetHeader(request, "ssl_cipher");
        if (strcert0 != null) {
            request.setAttribute(Globals.getCipherSuiteAttr(), strcert0);
        }
        strcert0 = mygetHeader(request, "ssl_session_id");
        if (strcert0 != null) {
            request.setAttribute(Globals.getSslSessionIdAttr(), strcert0);
            request.setAttribute(Globals.getSslSessionIdTomcatAttr(), strcert0);
        }
        strcert0 = mygetHeader(request, "ssl_cipher_usekeysize");
        if (strcert0 != null) {
            request.setAttribute(Globals.getKeySizeAttr(),
