...
public final class ClassLoaderFactory {
     ...
     * @exception Exception if an error occurs constructing the class loader
     */
    public static ClassLoader createClassLoader(List<ClassLoaderFactoryRepository> repositories,
                                                final ClassLoader parent)
        throws Exception {
        ...
        if (repositories != null) {
            for (ClassLoaderFactoryRepository repository : repositories)  {
                if (repository.getType() == ClassLoaderFactoryRepositoryType.URL) {
                    URL url = new URL(repository.getLocation());
                    if (log.isDebugEnabled())
                        log.debug("  Including URL " + url);
                    set.add(url);
                } else if (repository.getType() == ClassLoaderFactoryRepositoryType.DIR) {
                    File directory = new File(repository.getLocation());
                    directory = directory.getCanonicalFile();
                    if (!validateFile(directory, ClassLoaderFactoryRepositoryType.DIR)) {
                        continue;
                    }
                    ...
                    if (log.isDebugEnabled())
                        log.debug("  Including directory " + url);
                    set.add(url);
                } else if (repository.getType() == ClassLoaderFactoryRepositoryType.JAR) {
                    File file=new File(repository.getLocation());
                    file = file.getCanonicalFile();
                    if (!validateFile(file, ClassLoaderFactoryRepositoryType.JAR)) {
                        continue;
                    }
                    URL url = file.toURI().toURL();
                    if (log.isDebugEnabled())
                    ...
                    set.add(url);
                } else if (repository.getType() == ClassLoaderFactoryRepositoryType.GLOB) {
                    File directory=new File(repository.getLocation());
                    directory = directory.getCanonicalFile();
                    if (!validateFile(directory, ClassLoaderFactoryRepositoryType.GLOB)) {
                        continue;
                    }
                    if (log.isDebugEnabled())
    ...
    private static boolean validateFile(File file,
    		ClassLoaderFactoryRepositoryType type) throws IOException {
        if (ClassLoaderFactoryRepositoryType.DIR == type || ClassLoaderFactoryRepositoryType.GLOB == type) {
        ...
        } else if (ClassLoaderFactoryRepositoryType.JAR == type) {
            if (!file.exists() || !file.canRead()) {
