 ...
 */
public final class ApplicationDispatcher implements AsyncDispatcher,
		RequestDispatcher {

	private static final boolean STRICT_SERVLET_COMPLIANCE;

	private static final boolean WRAP_SAME_OBJECT;

	static {
		STRICT_SERVLET_COMPLIANCE = Globals.isStrictServletCompliance();
	...
	private static final StringManager3 sm = StringManager3
			.getManager(Constants3.getPackage());
	...
	public void forward(ServletRequest request, ServletResponse response)
		...
		if (Globals.isSecurityEnabled()) {
			try {
				ApplicationDispatcherPrivilegedForward dp = new ApplicationDispatcherPrivilegedForward(
	...
	public void doForward(ServletRequest request, ServletResponse response)
			throws ServletException, IOException {
		...
		ApplicationDispatcherState state = new ApplicationDispatcherState(
		...
		if ((servletPath == null) && (pathInfo == null)) {
			...
			HttpServletRequest hrequest = state.getHrequest();
	...
	private void processRequest(ServletRequest request,
			ServletResponse response, ApplicationDispatcherState state)
			throws IOException, ServletException {

		DispatcherType disInt = (DispatcherType) request.getAttribute(Globals
		...
		if (disInt != null) {
			...
			if (doInvoke) {
				if (disInt != DispatcherType.ERROR) {
					state.getOuterRequest().setAttribute(
							Globals.getDispatcherRequestPathAttr(),
							getCombinedPath());
					state.getOuterRequest().setAttribute(
							Globals.getDispatcherTypeAttr(),
							DispatcherType.FORWARD);
					invoke(state.getOuterRequest(), response, state);
				} else {
					invoke(state.getOuterRequest(), response, state);
				}
	...
	public void include(ServletRequest request, ServletResponse response)
		...
		if (Globals.isSecurityEnabled()) {
			try {
				ApplicationDispatcherPrivilegedInclude dp = new ApplicationDispatcherPrivilegedInclude(
						this, request, response);
				AccessController.doPrivileged(dp);
			} catch (PrivilegedActionException pe) {
				Exception e = pe.getException();
	...
	public void doInclude(ServletRequest request, ServletResponse response)
			throws ServletException, IOException {
		...
		if (name != null) {
			...
			ApplicationHttpRequest wrequest = (ApplicationHttpRequest) wrapRequest(state);
			wrequest.setAttribute(Globals.getNamedDispatcherAttr(), name);
			if (servletPath != null)
				wrequest.setServletPath(servletPath);
			wrequest.setAttribute(Globals.getDispatcherTypeAttr(),
					DispatcherType.INCLUDE);
			wrequest.setAttribute(Globals.getDispatcherRequestPathAttr(),
					getCombinedPath());
			invoke(state.getOuterRequest(), state.getOuterResponse(), state);
		}
	...
	public void dispatch(ServletRequest request, ServletResponse response)
		...
		if (Globals.isSecurityEnabled()) {
			try {
				ApplicationDispatcherPrivilegedDispatch dp = new ApplicationDispatcherPrivilegedDispatch(
						this, request, response);
				AccessController.doPrivileged(dp);
			} catch (PrivilegedActionException pe) {
				Exception e = pe.getException();
	...
	private void invoke(ServletRequest request, ServletResponse response,
			ApplicationDispatcherState state) throws IOException,
			ServletException {
		...
		HttpServletResponse hresponse = state.getHresponse();
		...
		try {
			support.fireInstanceEvent(InstanceEvent.getBeforeDispatchEvent(),
			...
			support.fireInstanceEvent(InstanceEvent.getAfterDispatchEvent(),
					servlet, request, response);
	 ...
	 */
	private void unwrapRequest(ApplicationDispatcherState state) {

		if (state.getWrapRequest() == null)
			return;

		if (state.getOuterRequest().isAsyncStarted()) {
			if (!state.getOuterRequest().getAsyncContext()
		...
		ServletRequest current = state.getOuterRequest();
		while (current != null) {
			...
			if (current == state.getWrapRequest()) {
				ServletRequest next = ((ServletRequestWrapper) current)
						.getRequest();
				if (previous == null)
					state.setOuterRequest(next);
				else
	 ...
	 */
	private void unwrapResponse(ApplicationDispatcherState state) {

		if (state.getWrapResponse() == null)
			return;
		...
		ServletResponse current = state.getOuterResponse();
		while (current != null) {
			...
			if (current == state.getWrapResponse()) {
				ServletResponse next = ((ServletResponseWrapper) current)
						.getResponse();
				if (previous == null)
					state.setOuterResponse(next);
				else
	 ...
	 */
	private ServletRequest wrapRequest(ApplicationDispatcherState state) {

		// Locate the request we should insert in front of
		ServletRequest previous = null;
		ServletRequest current = state.getOuterRequest();
		while (current != null) {
			if (state.getHrequest() == null
					&& (current instanceof HttpServletRequest))
				state.setHrequest((HttpServletRequest) current);
		...
		if ((current instanceof ApplicationHttpRequest)
			...
			if ((state.getOuterRequest() instanceof ApplicationHttpRequest)
					|| (state.getOuterRequest() instanceof Request)
					|| (state.getOuterRequest() instanceof HttpServletRequest)) {
				HttpServletRequest houterRequest = (HttpServletRequest) state.getOuterRequest();
		...
		if (previous == null)
			state.setOuterRequest(wrapper);
		else
			((ServletRequestWrapper) previous).setRequest(wrapper);
		state.setWrapRequest(wrapper);
		return (wrapper);
	 ...
	 */
	private ServletResponse wrapResponse(ApplicationDispatcherState state) {

		// Locate the response we should insert in front of
		ServletResponse previous = null;
		ServletResponse current = state.getOuterResponse();
		while (current != null) {
			if (state.getHresponse() == null
					&& (current instanceof HttpServletResponse)) {
				state.setHresponse((HttpServletResponse) current);
				if (!state.isIncluding()) // Forward only needs hresponse
					return null;
			}
		...
		if ((current instanceof ApplicationHttpResponse)
			...
			wrapper = new ApplicationHttpResponse(
					(HttpServletResponse) current, state.isIncluding());
		else
			wrapper = new ApplicationResponse(current, state.isIncluding());
		if (previous == null)
			state.setOuterResponse(wrapper);
		else
			((ServletResponseWrapper) previous).setResponse(wrapper);
		state.setWrapResponse(wrapper);
		return (wrapper);
	...
	private void recycleRequestWrapper(ApplicationDispatcherState state) {
		if (state.getWrapRequest() instanceof ApplicationHttpRequest) {
			((ApplicationHttpRequest) state.getWrapRequest()).recycle();
		}
	}

	public static boolean isWrapSameObject() {
		return WRAP_SAME_OBJECT;
	}
