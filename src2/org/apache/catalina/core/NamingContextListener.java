...
public class NamingContextListener
     ...
     */
    private HashMap<String, ObjectName> objectNames =
        ...
        if (Lifecycle.CONFIGURE_START_EVENT.equals(event.getType())) {
            ...
            try {
                ...
                if (!objectNames.isEmpty()) {
                    ...
                    Registry2 registry = Registry2.getRegistry(null, null);
    ...
    public void addService(ContextService service) {
        ...
        while (portcomponent.hasNext()) {
            ...
            StringRefAddr refAddr = new StringRefAddr(ServiceRef.getServiceendpointinterface(), serviceendpoint);
            ref.add(refAddr);
            String portlink = service.getPortlink(serviceendpoint);
            refAddr = new StringRefAddr(ServiceRef.getPortcomponentlink(), portlink);
            ref.add(refAddr);
        }
        // Adding the additional parameters, if any
        ...
        while (handlers.hasNext()) {
            ...
            while (localParts.hasNext()) {
                ...
                handlerRef.add(new StringRefAddr(HandlerRef.getHandlerLocalpart(), localPart));
                handlerRef.add(new StringRefAddr(HandlerRef.getHandlerNamespace(), namespaceURI));
            }
            Iterator<String> params = handler.listProperties();
            while (params.hasNext()) {
                String paramName = params.next();
                String paramValue = (String) handler.getProperty(paramName);
                handlerRef.add(new StringRefAddr(HandlerRef.getHandlerParamname(), paramName));
                handlerRef.add(new StringRefAddr(HandlerRef.getHandlerParamvalue(), paramValue));
            }
            for (int i = 0; i < handler.getSoapRolesSize(); i++) {
                handlerRef.add(new StringRefAddr(HandlerRef.getHandlerSoaprole(), handler.getSoapRole(i)));
            }
            for (int i = 0; i < handler.getPortNamesSize(); i++) {
                handlerRef.add(new StringRefAddr(HandlerRef.getHandlerPortname(), handler.getPortName(i)));
            }
    ...
    public void addResource(ContextResource resource) {
        ...
        if ("javax.sql.DataSource".equals(ref.getClassName()) &&
            ...
            try {
                ...
                Registry2.getRegistry(null, null).registerComponent(actualResource, on, null);
    ...
    public void removeResource(String name) {
        ...
        if (on != null) {
            Registry2.getRegistry(null, null).unregisterComponent(on);
        }
