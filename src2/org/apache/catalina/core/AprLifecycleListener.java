...
public class AprLifecycleListener implements LifecycleListener {
	 ...
	 */
	private static final List<String> initInfoLogMessages = new ArrayList<String>(
			3);

	/**
	 * The string manager for this package.
	 */
	private static final StringManager3 sm = StringManager3.getManager(Constants3
			.getPackage());
	...
	private static final int TCN_REQUIRED_PATCH = 32;
	private static final int TCN_RECOMMENDED_MINOR = 1;
	private static final int TCN_RECOMMENDED_PV = 32;

	// ---------------------------------------------- Properties
	private static String SSLEngine = "on"; // default on
	private static String FIPSMode = "off"; // default off, valid only when
											// SSLEngine="on"
	private static String SSLRandomSeed = "builtin";
	private static boolean sslInitialized = false;
	private static boolean aprInitialized = false;
	@Deprecated
	private static boolean sslAvailable = false;
	private static boolean aprAvailable = false;
	private static boolean fipsModeActive = false;
	...
	private static void init() {
		...
		if (apver < rcver) {
			initInfoLogMessages
					.add(sm.getString("aprListener.tcnVersion", major + "."
							+ minor + "." + patch, TCN_REQUIRED_MAJOR + "."
							+ TCN_RECOMMENDED_MINOR + "." + TCN_RECOMMENDED_PV));
		}

		initInfoLogMessages.add(sm.getString(
				"aprListener.tcnValid",
				major + "." + minor + "." + patch,
				Library.getAPR_MAJOR_VERSION() + "."
						+ Library.getAPR_MINOR_VERSION() + "."
						+ Library.getAPR_PATCH_VERSION()));

		// Log APR flags
		initInfoLogMessages.add(sm.getString("aprListener.flags",
				Boolean.valueOf(Library.isAPR_HAVE_IPV6()),
				Boolean.valueOf(Library.isAPR_HAS_SENDFILE()),
				Boolean.valueOf(Library.isAPR_HAS_SO_ACCEPTFILTER()),
				Boolean.valueOf(Library.isAPR_HAS_RANDOM())));
		aprAvailable = true;
	}
