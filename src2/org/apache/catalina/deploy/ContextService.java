/*
 * Licensed to the Apache Software Foundation (ASF) under one or more
 * contributor license agreements.  See the NOTICE file distributed with
 * this work for additional information regarding copyright ownership.
 * The ASF licenses this file to You under the Apache License, Version 2.0
 * (the "License"); you may not use this file except in compliance with
 * the License.  You may obtain a copy of the License at
 * 
 *      http://www.apache.org/licenses/LICENSE-2.0
 * 
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package org.apache.catalina.deploy;

import java.util.Arrays;
import java.util.HashMap;
import java.util.Iterator;

/**
 * Representation of a web service reference for a web application, as
 * represented in a <code>&lt;service-ref&gt;</code> element in the
 * deployment descriptor.
 *
 * @author Fabien Carrion
 */
public class ContextService extends ResourceBase {

    private static final long serialVersionUID = 1L;

    // ------------------------------------------------------------- Properties


    /**
     * The WebService reference name.
     */
    private String displayname = null;

    public String getDisplayname() {
        return (this.getDisplaynameData());
    }

    public void setDisplayname(String displayname) {
        this.setDisplaynameData(displayname);
    }

    /**
     * A large icon for this WebService.
     */
    private String largeIcon = null;

    public String getLargeIcon() {
        return (this.getLargeIconData());
    }

    public void setLargeIcon(String largeIcon) {
        this.setLargeIconData(largeIcon);
    }

    /**
     * A small icon for this WebService.
     */
    private String smallIcon = null;

    public String getSmallIcon() {
        return (this.getSmallIconData());
    }

    public void setSmallIcon(String smallIcon) {
        this.setSmallIconData(smallIcon);
    }

    /**
     * The fully qualified class name of the JAX-WS Service interface that the
     * client depends on.
     */
    private String serviceInterface = null;
    
    public String getInterface() {
        return getServiceInterfaceData();
    }
    
    public void setInterface(String serviceInterface) {
        this.setServiceInterfaceData(serviceInterface);
    }

    /**
     * Contains the location (relative to the root of
     * the module) of the web service WSDL description.
     */
    private String wsdlfile = null;

    public String getWsdlfile() {
        return (this.getWsdlfileData());
    }

    public void setWsdlfile(String wsdlfile) {
        this.setWsdlfileData(wsdlfile);
    }

    /**
     * A file specifying the correlation of the WSDL definition
     * to the interfaces (Service Endpoint Interface, Service Interface). 
     */
    private String jaxrpcmappingfile = null;

    public String getJaxrpcmappingfile() {
        return (this.getJaxrpcmappingfileData());
    }

    public void setJaxrpcmappingfile(String jaxrpcmappingfile) {
        this.setJaxrpcmappingfileData(jaxrpcmappingfile);
    }

    /**
     * Declares the specific WSDL service element that is being referred to.
     * It is not specified if no wsdl-file is declared or if WSDL contains only
     * 1 service element.
     *
     * A service-qname is composed by a namespaceURI and a localpart.
     * It must be defined if more than 1 service is declared in the WSDL.
     *
     * serviceqname[0] : namespaceURI
     * serviceqname[1] : localpart
     */
    private String[] serviceqname = new String[2];

    public String[] getServiceqname() {
        return (this.getServiceqnameData());
    }

    public String getServiceqname(int i) {
        return this.getServiceqnameData()[i];
    }

    public String getServiceqnameNamespaceURI() {
        return this.getServiceqnameData()[0];
    }

    public String getServiceqnameLocalpart() {
        return this.getServiceqnameData()[1];
    }

    public void setServiceqname(String[] serviceqname) {
        this.setServiceqnameData(serviceqname);
    }

    public void setServiceqname(String serviceqname, int i) {
        this.getServiceqnameData()[i] = serviceqname;
    }

    public void setServiceqnameNamespaceURI(String namespaceuri) {
        this.getServiceqnameData()[0] = namespaceuri;
    }

    public void setServiceqnameLocalpart(String localpart) {
        this.getServiceqnameData()[1] = localpart;
    }

    /**
     * Declares a client dependency on the container to resolving a Service Endpoint Interface
     * to a WSDL port. It optionally associates the Service Endpoint Interface with a
     * particular port-component.
     *
     */
    public Iterator<String> getServiceendpoints() {
        return this.listProperties();
    }

    public String getPortlink(String serviceendpoint) {
        return (String) this.getProperty(serviceendpoint);
    }

    public void addPortcomponent(String serviceendpoint, String portlink) {
        if (portlink == null)
            portlink = "";
        this.setProperty(serviceendpoint, portlink);
    }

    /**
     * A list of Handlers to use for this service-ref.
     *
     * The instantiation of the handler have to be done.
     */
    private final HashMap<String, ContextHandler> handlers =
        new HashMap<String, ContextHandler>();

    public Iterator<String> getHandlers() {
        return getHandlersData().keySet().iterator();
    }

    public ContextHandler getHandler(String handlername) {
        return getHandlersData().get(handlername);
    }

    public void addHandler(ContextHandler handler) {
        getHandlersData().put(handler.getName(), handler);
    }


    // --------------------------------------------------------- Public Methods


    /**
     * Return a String representation of this object.
     */
    @Override
    public String toString() {

        StringBuilder sb = new StringBuilder("ContextService[");
        sb.append("name=");
        sb.append(getName());
        if (getDescription() != null) {
            sb.append(", description=");
            sb.append(getDescription());
        }
        if (getType() != null) {
            sb.append(", type=");
            sb.append(getType());
        }
        if (getDisplaynameData() != null) {
            sb.append(", displayname=");
            sb.append(getDisplaynameData());
        }
        if (getLargeIconData() != null) {
            sb.append(", largeIcon=");
            sb.append(getLargeIconData());
        }
        if (getSmallIconData() != null) {
            sb.append(", smallIcon=");
            sb.append(getSmallIconData());
        }
        if (getWsdlfileData() != null) {
            sb.append(", wsdl-file=");
            sb.append(getWsdlfileData());
        }
        if (getJaxrpcmappingfileData() != null) {
            sb.append(", jaxrpc-mapping-file=");
            sb.append(getJaxrpcmappingfileData());
        }
        if (getServiceqnameData()[0] != null) {
            sb.append(", service-qname/namespaceURI=");
            sb.append(getServiceqnameData()[0]);
        }
        if (getServiceqnameData()[1] != null) {
            sb.append(", service-qname/localpart=");
            sb.append(getServiceqnameData()[1]);
        }
        if (this.getServiceendpoints() != null) {
            sb.append(", port-component/service-endpoint-interface=");
            sb.append(this.getServiceendpoints());
        }
        if (getHandlersData() != null) {
            sb.append(", handler=");
            sb.append(getHandlersData());
        }
        sb.append("]");
        return (sb.toString());
    }


    @Override
    public int hashCode() {
        final int prime = 31;
        int result = super.hashCode();
        result = prime * result +
                ((getDisplaynameData() == null) ? 0 : getDisplaynameData().hashCode());
        result = prime * result +
                ((getHandlersData() == null) ? 0 : getHandlersData().hashCode());
        result = prime *
                result +
                ((getJaxrpcmappingfileData() == null) ? 0 : getJaxrpcmappingfileData().hashCode());
        result = prime * result +
                ((getLargeIconData() == null) ? 0 : getLargeIconData().hashCode());
        result = prime * result +
                ((getServiceInterfaceData() == null) ? 0 : getServiceInterfaceData().hashCode());
        result = prime * result + Arrays.hashCode(getServiceqnameData());
        result = prime * result +
                ((getSmallIconData() == null) ? 0 : getSmallIconData().hashCode());
        result = prime * result +
                ((getWsdlfileData() == null) ? 0 : getWsdlfileData().hashCode());
        return result;
    }


    @Override
    public boolean equals(Object obj) {
        if (this == obj) {
            return true;
        }
        if (!super.equals(obj)) {
            return false;
        }
        if (getClass() != obj.getClass()) {
            return false;
        }
        ContextService other = (ContextService) obj;
        if (getDisplaynameData() == null) {
            if (other.getDisplaynameData() != null) {
                return false;
            }
        } else if (!getDisplaynameData().equals(other.getDisplaynameData())) {
            return false;
        }
        if (getHandlersData() == null) {
            if (other.getHandlersData() != null) {
                return false;
            }
        } else if (!getHandlersData().equals(other.getHandlersData())) {
            return false;
        }
        if (getJaxrpcmappingfileData() == null) {
            if (other.getJaxrpcmappingfileData() != null) {
                return false;
            }
        } else if (!getJaxrpcmappingfileData().equals(other.getJaxrpcmappingfileData())) {
            return false;
        }
        if (getLargeIconData() == null) {
            if (other.getLargeIconData() != null) {
                return false;
            }
        } else if (!getLargeIconData().equals(other.getLargeIconData())) {
            return false;
        }
        if (getServiceInterfaceData() == null) {
            if (other.getServiceInterfaceData() != null) {
                return false;
            }
        } else if (!getServiceInterfaceData().equals(other.getServiceInterfaceData())) {
            return false;
        }
        if (!Arrays.equals(getServiceqnameData(), other.getServiceqnameData())) {
            return false;
        }
        if (getSmallIconData() == null) {
            if (other.getSmallIconData() != null) {
                return false;
            }
        } else if (!getSmallIconData().equals(other.getSmallIconData())) {
            return false;
        }
        if (getWsdlfileData() == null) {
            if (other.getWsdlfileData() != null) {
                return false;
            }
        } else if (!getWsdlfileData().equals(other.getWsdlfileData())) {
            return false;
        }
        return true;
    }

	public String getDisplaynameData() {
		return displayname;
	}

	public void setDisplaynameData(String displayname) {
		this.displayname = displayname;
	}

	public String getLargeIconData() {
		return largeIcon;
	}

	public void setLargeIconData(String largeIcon) {
		this.largeIcon = largeIcon;
	}

	public String getSmallIconData() {
		return smallIcon;
	}

	public void setSmallIconData(String smallIcon) {
		this.smallIcon = smallIcon;
	}

	public String getServiceInterfaceData() {
		return serviceInterface;
	}

	public void setServiceInterfaceData(String serviceInterface) {
		this.serviceInterface = serviceInterface;
	}

	public String getWsdlfileData() {
		return wsdlfile;
	}

	public void setWsdlfileData(String wsdlfile) {
		this.wsdlfile = wsdlfile;
	}

	public String getJaxrpcmappingfileData() {
		return jaxrpcmappingfile;
	}

	public void setJaxrpcmappingfileData(String jaxrpcmappingfile) {
		this.jaxrpcmappingfile = jaxrpcmappingfile;
	}

	public String[] getServiceqnameData() {
		return serviceqname;
	}

	public void setServiceqnameData(String[] serviceqname) {
		this.serviceqname = serviceqname;
	}

	public HashMap<String, ContextHandler> getHandlersData() {
		return handlers;
	}
}
