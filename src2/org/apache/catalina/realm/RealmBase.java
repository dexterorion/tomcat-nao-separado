...
import org.apache.tomcat.util.codec.binary.Base642;
...
public abstract class RealmBase extends LifecycleMBeanBase implements Realm {
	 ...
	 */
	private static final String info = "org.apache.catalina.realm.RealmBase/1.0";
	 ...
	 */
	private static final StringManager3 sm = StringManager3.getManager(Constants13
			.getPackage());
	 ...
	 */
	private X509UsernameRetriever x509UsernameRetriever;

	/**
	 * The all role mode.
	 */
	private RealmBaseAllRolesMode allRolesMode = RealmBaseAllRolesMode.getStrictMode();

	/**
	 * When processing users authenticated via the GSS-API, should any
	 * &quot;@...&quot; be stripped from the end of the user name?
	 */
	private boolean stripRealmForGss = true;
	...
	public RealmBaseAllRolesMode getAllRolesModeVariable() {
	...
	public void setAllRolesMode(String allRolesMode) {
		...
		this.allRolesMode = RealmBaseAllRolesMode.toMode(allRolesMode);
	...
	protected boolean compareCredentials(String userCredentials,
		...
		if (hasMessageDigest()) {
			...
			if (serverCredentials.startsWith("{MD5}")
				...
				synchronized (this) {
					md.reset();
					md.update(userCredentials.getBytes(B2CConverter
							.getIso88591()));
					userDigest = Base642.encodeBase64String(md.digest());
				}
				...
				// digested password.
				byte[] serverDigestPlusSaltBytes = Base642
	...
	public boolean hasResourcePermission(Request request, Response response,
		...
		if (!denyfromall && allRolesMode != RealmBaseAllRolesMode.getStrictMode()
			...
			for (int i = 0; i < constraints.length; i++) {
				...
				if (constraint.getAllRoles()) {
					if (allRolesMode == RealmBaseAllRolesMode.getAuthOnlyMode()) {
						if (log.isDebugEnabled()) {
					...
					roles = request.getContext().findSecurityRoles();
					if (roles.length == 0
							&& allRolesMode == RealmBaseAllRolesMode.getStrictAuthOnlyMode()) {
						if (log.isDebugEnabled()) {
	...
	public boolean hasUserDataPermission(Request request, Response response,
		...
		for (int i = 0; i < constraints.length; i++) {
			...
			if (userConstraint.equals(Constants13.getNoneTransport())) {
				if (log.isDebugEnabled())
		...
		if ((requestedSessionId != null)
			...
			file.append(";");
			file.append(SessionConfig2.getSessionUriParamName(request
					.getContext()));
			file.append("=");
	...
	private String realmPath = "/realm0";
	...
	public static MessageDigest getMd5Helper() {
		return md5Helper;
	}

	public static void setMd5Helper(MessageDigest md5Helper) {
		RealmBase.md5Helper = md5Helper;
	}
	...
	public X509UsernameRetriever getX509UsernameRetriever() {
		return x509UsernameRetriever;
	}

	public void setX509UsernameRetriever(
			X509UsernameRetriever x509UsernameRetriever) {
		this.x509UsernameRetriever = x509UsernameRetriever;
	}
	...
	public static MD5Encoder getMd5encoder() {
		return md5Encoder;
	}
	...
	public void setAllRolesMode(RealmBaseAllRolesMode allRolesMode) {
