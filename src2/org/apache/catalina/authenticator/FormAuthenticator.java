...
public class FormAuthenticator extends AuthenticatorBase {
    ...
    public boolean authenticate(Request request,
        ...
        String ssoId = (String) request.getNote(Constants.getReqSsoidNote());
        if (principal != null) {
        ...
        if (!getCache()) {
            ...
            String username =
                (String) session.getNote(Constants.getSessUsernameNote());
            String password =
                (String) session.getNote(Constants.getSessPasswordNote());
            if ((username != null) && (password != null)) {
                if (log.isDebugEnabled()) {
                    log.debug("Reauthenticating username '" + username + "'");
                }
                principal =
                    getContext().getRealm().authenticate(username, password);
                if (principal != null) {
                    session.setNote(Constants.getFormPrincipalNote(), principal);
        ...
        if (matchRequest(request)) {
            ...
            principal = (Principal)
                session.getNote(Constants.getFormPrincipalNote());
            ...
            if (getCache()) {
                session.removeNote(Constants.getSessUsernameNote());
                session.removeNote(Constants.getSessPasswordNote());
            }
        ...
        boolean loginAction =
            ...
            requestURI.endsWith(Constants.getFormAction());
        ...
        if (!loginAction) {
            ...
            try {
                ...
                response.sendError(HttpServletResponse.SC_FORBIDDEN,
                        getSm().getString("authenticator.requestBodyTooBig"));
                return (false);
            }
        ...
        String username = request.getParameter(Constants.getFormUsername());
        String password = request.getParameter(Constants.getFormPassword());
        if (log.isDebugEnabled()) {
        ...
        if (session == null) {
            if (getContainerLog().isDebugEnabled()) {
                getContainerLog().debug
                    ("User took so long to log on the session expired");
            }
            if (landingPage == null) {
                response.sendError(HttpServletResponse.SC_REQUEST_TIMEOUT,
                        getSm().getString("authenticator.sessionExpired"));
            } else {
                ...
                request.getSessionInternal(true).setNote(
                        Constants.getFormRequestNote(), saved);
        ...
        session.setNote(Constants.getSessUsernameNote(), username);
        session.setNote(Constants.getSessPasswordNote(), password);
        ...
        if (requestURI == null) {
            if (landingPage == null) {
                response.sendError(HttpServletResponse.SC_BAD_REQUEST,
                        getSm().getString("authenticator.formlogin"));
            } else {
                // Make the authenticator think the user originally requested
                // the landing page
                String uri = request.getContextPath() + landingPage;
                ...
                saved.setRequestURI(uri);
                saved.setDecodedRequestURI(uri);
                session.setNote(Constants.getFormRequestNote(), saved);
                response.sendRedirect(response.encodeRedirectURL(uri));
            }
        } else {
    ...
    protected void forwardToLoginPage(Request request,
        ...
        if (log.isDebugEnabled()) {
            log.debug(getSm().getString("formAuthenticator.forwardLogin",
                    request.getRequestURI(), request.getMethod(),
                    config.getLoginPage(), getContext().getName()));
        }

        String loginPage = config.getLoginPage();
        if (loginPage == null || loginPage.length() == 0) {
            String msg = getSm().getString("formAuthenticator.noLoginPage",
                    getContext().getName());
            log.warn(msg);
        ...
        RequestDispatcher disp =
            getContext().getServletContext().getRequestDispatcher(loginPage);
        try {
            if (getContext().fireRequestInitEvent(request)) {
                disp.forward(request.getRequest(), response);
                getContext().fireRequestDestroyEvent(request);
            }
        } catch (Throwable t) {
            ExceptionUtils2.handleThrowable(t);
            String msg = getSm().getString("formAuthenticator.forwardLoginFail");
            log.warn(msg, t);
    ...
    protected void forwardToErrorPage(Request request,
        ...
        if (errorPage == null || errorPage.length() == 0) {
            String msg = getSm().getString("formAuthenticator.noErrorPage",
                    getContext().getName());
            log.warn(msg);
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
                    msg);
        ...
        RequestDispatcher disp =
            getContext().getServletContext().getRequestDispatcher
            (config.getErrorPage());
        try {
            if (getContext().fireRequestInitEvent(request)) {
                disp.forward(request.getRequest(), response);
            ...
            ExceptionUtils2.handleThrowable(t);
            String msg = getSm().getString("formAuthenticator.forwardErrorFail");
            log.warn(msg, t);
            request.setAttribute(RequestDispatcher.ERROR_EXCEPTION, t);
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
                    msg);
    ...
    protected boolean matchRequest(Request request) {
      ...
      SavedRequest sreq = (SavedRequest)
          session.getNote(Constants.getFormRequestNote());
      if (sreq == null) {
        return (false);
    }

      // Is there a saved principal?
      if (session.getNote(Constants.getFormPrincipalNote()) == null) {
        return (false);
    }
     ...
     */
    protected boolean restoreRequest(Request request, Session2 session)
            throws IOException {

        // Retrieve and remove the SavedRequest object from our session
        SavedRequest saved = (SavedRequest)
            session.getNote(Constants.getFormRequestNote());
        session.removeNote(Constants.getFormRequestNote());
        session.removeNote(Constants.getFormPrincipalNote());
        if (saved == null) {
            return (false);
        }
     ...
     * @throws IOException
     */
    protected void saveRequest(Request request, Session2 session)
        throws IOException {
     ...
     */
    protected String savedRequestURL(Session2 session) {

        SavedRequest saved =
            (SavedRequest) session.getNote(Constants.getFormRequestNote());
        if (saved == null) {
            return (null);
        }
