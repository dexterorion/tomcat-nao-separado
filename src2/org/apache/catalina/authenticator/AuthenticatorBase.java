...
public abstract class AuthenticatorBase extends ValveBase implements
	 ...
	 */
	private static final String REALM_NAME = "Authentication required";
	 ...
	 * created.
	 */
	private boolean alwaysUseSession = false;
	 ...
	 */
	private static final String info = "org.apache.catalina.authenticator.AuthenticatorBase/1.0";

	/**
	 * Flag to determine if we disable proxy caching, or leave the issue up to
	 * the webapp developer.
	 */
	private boolean disableProxyCaching = true;

	/**
	 * Flag to determine if we disable proxy caching with headers incompatible
	 * with IE.
	 */
	private boolean securePagesWithPragma = false;
	...
	private static final StringManager3 sm = StringManager3
			.getManager(Constants.getPackage());
	...
	private static final String DATE_ONE = (new SimpleDateFormat(
			DateTool.getHttpResponseDateHeader(), Locale.US)).format(new Date(1));
	...
	public void invoke(Request request, Response response) throws IOException,
		...
		if (session != null) {
			SavedRequest savedRequest = (SavedRequest) session
					.getNote(Constants.getFormRequestNote());
		...
		Wrapper wrapper = (Wrapper) request.getMappingData().getWrapper();
		if (wrapper != null) {
	...
	protected X509Certificate[] getRequestCertificates(final Request request)
		...
		if ((certs == null) || (certs.length < 1)) {
			try {
				...
				certs = (X509Certificate[]) request.getAttribute(Globals
						.getCertificatesAttr());
	...
	public void register(Request request, HttpServletResponse response,
		...
		if (ssoId == null) {
			...
			Cookie cookie = new Cookie(Constants.getSingleSignOnCookie(), ssoId);
			cookie.setMaxAge(-1);
			cookie.setPath("/");
			...
			request.setNote(Constants.getReqSsoidNote(), ssoId);
	...
	public SingleSignOn getSso() {
		return sso;
	}

	public void setSso(SingleSignOn sso) {
		this.sso = sso;
	}
