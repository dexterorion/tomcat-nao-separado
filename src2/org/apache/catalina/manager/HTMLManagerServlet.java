...
import org.apache.catalina.manager.util.BaseSessionComparator1;
import org.apache.catalina.manager.util.BaseSessionComparator10;
import org.apache.catalina.manager.util.BaseSessionComparator2;
import org.apache.catalina.manager.util.BaseSessionComparator3;
import org.apache.catalina.manager.util.BaseSessionComparator4;
import org.apache.catalina.manager.util.BaseSessionComparator5;
import org.apache.catalina.manager.util.BaseSessionComparator6;
import org.apache.catalina.manager.util.BaseSessionComparator7;
import org.apache.catalina.manager.util.BaseSessionComparator8;
import org.apache.catalina.manager.util.BaseSessionComparator9;
...
public final class HTMLManagerServlet extends ManagerServlet {
    ...
    private static final String APPLICATION_ERROR = "error";
    
    private static final String sessionsListJspPath  = "/WEB-INF/jsp/sessionsList.jsp";
    private static final String sessionDetailJspPath = "/WEB-INF/jsp/sessionDetail.jsp";
    ...
    public void doGet(HttpServletRequest request,
        ...
        StringManager3 smClient = StringManager3.getManager(
                Constants10.getPackage(), request.getLocales());
        ...
        response.setContentType("text/html; charset=" + Constants10.getCharset());
    ...
    protected String upload(HttpServletRequest request, StringManager3 smClient) {
        String message = "";
        ...
        try {
            while (true) {
                ...
                // (if any)
                File file = new File(getDeployed(), filename);
                if (file.exists()) {
                    message = smClient.getString(
                ...
                if ((getHost().findChild(name) != null) && !isDeployed(name)) {
                    message = smClient.getString(
    ...
    protected void list(HttpServletRequest request,
                     HttpServletResponse response,
                     String message,
                     StringManager3 smClient) throws IOException {
        ...
        // HTML Header Section
        writer.print(Constants10.getHtmlHeaderSection());
        ...
        writer.print(MessageFormat.format
                     (Constants10.getBodyHeaderSection(), args));
        ...
        writer.print(MessageFormat.format(Constants10.getMessageSection(), args));
        ...
        writer.print(MessageFormat.format(Constants10.getManagerSection(), args));
        ...
        for (String contextName : contextNames) {
            Context ctxt = (Context) getHost().findChild(contextName);
            ...
            if (ctxt != null) {
                ...
                if (ctxt.getName().equals(this.getContext().getName())) {
                    writer.print(MessageFormat.format(
        ...
        args[7] = getSm().getString("htmlManagerServlet.serverHostname");
        args[8] = getSm().getString("htmlManagerServlet.serverIPAddress");
        writer.print(MessageFormat.format
                     (Constants10.getServerHeaderSection(), args));
        ...
        writer.print(MessageFormat.format(Constants10.getServerRowSection(), args));

        // HTML Tail Section
        writer.print(Constants10.getHtmlTailSection());
     ...
     *
     * @see ManagerServlet#reload(PrintWriter, ContextName, StringManager3)
     *
     * @param cn Name of the application to be restarted
     * @param smClient  StringManager for the client's locale
     * @return message String
     */
    protected String reload(ContextName cn, StringManager3 smClient) {
     ...
     *
     * @see ManagerServlet#undeploy(PrintWriter, ContextName, StringManager3)
     *
     * @param cn Name of the application to be undeployed
     * @param smClient  StringManager for the client's locale
     * @return message String
     */
    protected String undeploy(ContextName cn, StringManager3 smClient) {
     ...
     * @return message String
     */
    protected String sessions(ContextName cn, int idle, StringManager3 smClient) {
     ...
     *
     * @see ManagerServlet#start(PrintWriter, ContextName, StringManager3)
     *
     * @param cn Name of the application to be started
     * @param smClient  StringManager for the client's locale
     * @return message String
     */
    protected String start(ContextName cn, StringManager3 smClient) {
     ...
     *
     * @see ManagerServlet#stop(PrintWriter, ContextName, StringManager3)
     *
     * @param cn Name of the application to be stopped
     * @param smClient  StringManager for the client's locale
     * @return message String
     */
    protected String stop(ContextName cn, StringManager3 smClient) {
     ...
     *
     * @see ManagerServlet#findleaks(boolean, PrintWriter, StringManager3) 
     * 
     * @param smClient  StringManager for the client's locale
     *
     * @return message String
     */
    protected String findleaks(StringManager3 smClient) {
    ...
    protected void doSessions(ContextName cn, HttpServletRequest req,
            HttpServletResponse resp, StringManager3 smClient)
    ...
    protected List<Session2> getSessionsForName(ContextName cn,
            StringManager3 smClient) {
        ...
        Context ctxt = (Context) getHost().findChild(cn.getName());
        if (null == ctxt) {
            throw new IllegalArgumentException(smClient.getString(
        ...
        Manager manager = ctxt.getManager();
        List<Session2> sessions = new ArrayList<Session2>();
    ...
    protected Session2 getSessionForNameAndId(ContextName cn, String id,
            StringManager3 smClient) {

        List<Session2> sessions = getSessionsForName(cn, smClient);
    ...
    protected void displaySessionsListPage(ContextName cn,
        ...
        if (null != sortBy && !"".equals(sortBy.trim())) {
            Comparator<Session2> comparator = getComparator(sortBy);
            if (comparator != null) {
    ...
    protected void displaySessionDetailPage(HttpServletRequest req,
            ...
            StringManager3 smClient) throws ServletException, IOException {
        Session2 session = getSessionForNameAndId(cn, sessionId, smClient);
        //strong>NOTE</strong> - This header will be overridden
        // automatically if a <code>RequestDispatcher.forward()</code> call is
        // ultimately invoked.
        resp.setHeader("Pragma", "No-cache"); // HTTP 1.0
    ...
    protected boolean removeSessionAttribute(ContextName cn, String sessionId,
            String attributeName, StringManager3 smClient) throws IOException {
        HttpSession session =
            getSessionForNameAndId(cn, sessionId, smClient).getSession();
        if (null == session) {
            // Shouldn't happen, but let's play nice...
    ...
    protected Comparator<Session2> getComparator(String sortBy) {
        Comparator<Session2> comparator = null;
        if ("CreationTime".equalsIgnoreCase(sortBy)) {
            comparator = new BaseSessionComparator1();
        } else if ("id".equalsIgnoreCase(sortBy)) {
            comparator = new BaseSessionComparator2();
        } else if ("LastAccessedTime".equalsIgnoreCase(sortBy)) {
            comparator = new BaseSessionComparator3();
        } else if ("MaxInactiveInterval".equalsIgnoreCase(sortBy)) {
            comparator = new BaseSessionComparator4();
        } else if ("new".equalsIgnoreCase(sortBy)) {
            comparator = new BaseSessionComparator5();
        } else if ("locale".equalsIgnoreCase(sortBy)) {
            comparator = new BaseSessionComparator6();
        } else if ("user".equalsIgnoreCase(sortBy)) {
            comparator = new BaseSessionComparator7();
        } else if ("UsedTime".equalsIgnoreCase(sortBy)) {
            comparator = new BaseSessionComparator8();
        } else if ("InactiveTime".equalsIgnoreCase(sortBy)) {
            comparator = new BaseSessionComparator9();
        } else if ("TTL".equalsIgnoreCase(sortBy)) {
            comparator = new BaseSessionComparator10();
        }
