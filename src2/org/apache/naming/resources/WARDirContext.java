...
public class WARDirContext extends BaseDirContext {
	 ...
	 */
	protected WARDirContext(ZipFile base, WARDirContextEntry entries) {
		this.base = base;
	 ...
	 * The WAR file.
	 */
	private ZipFile base = null;

	/**
	 * WAR entries.
	 */
	private WARDirContextEntry entries = null;
	...
	protected Object doLookup(String strName) {
		...
		try {
			...
			log.info(getSm().getString("resources.invalidName", strName), e);
			return null;
		}

		if (name.isEmpty())
			return this;
		WARDirContextEntry entry = treeLookup(name);
		if (entry == null)
			return null;
		...
		if (zipEntry.isDirectory())
			...
			return new WARDirContextWARResource(this, entry.getEntry());
	}
	...
	protected void loadEntries() {
		...
		try {
			...
			entries = new WARDirContextEntry("/", new ZipEntry("/"));
			...
			while (entryList.hasMoreElements()) {
				...
				while ((currentPos = name.indexOf('/', lastPos)) != -1) {
					...
					// this loop
					WARDirContextEntry parent = treeLookup(parentName);
					WARDirContextEntry child = treeLookup(childName);
					if (child == null) {
						...
						child = new WARDirContextEntry(entryName, new ZipEntry(
								zipName));
						if (parent != null)
				...
				WARDirContextEntry parent = treeLookup(compositeName);
				WARDirContextEntry child = new WARDirContextEntry(entryName,
						entry);
				if (parent != null)
					parent.addChild(child);
	 ...
	 * Entry tree lookup.
	 */
	protected WARDirContextEntry treeLookup(Name name) {
		if (name.isEmpty() || entries == null)
			return entries;
		WARDirContextEntry currentEntry = entries;
	 ...
	 */
	protected ArrayList<NamingEntry> list(WARDirContextEntry entry) {

		ArrayList<NamingEntry> entries = new ArrayList<NamingEntry>();
		WARDirContextEntry[] children = entry.getChildren();
		...
		for (int i = 0; i < children.length; i++) {
			...
			if (current.isDirectory()) {
				...
				object = new WARDirContextWARResource(this, current);
			}
	...
	public ZipFile getBase() {
		return base;
	}

	public void setBase(ZipFile base) {
		this.base = base;
	}
