...
public class ProxyDirContext implements DirContext {
	 ...
	 */
	private static final NameNotFoundException NOT_FOUND_EXCEPTION = new ImmutableNameNotFoundException();
	 ...
	 */
	private ProxyDirContext proxy = this;
	 ...
	 */
	private static final StringManager2 sm = StringManager2.getManager(Constants31
			.getPackage());
	 ...
	 * Cache class.
	 */
	private String cacheClassName = "org.apache.naming.resources.ResourceCache";
	 ...
	 */
	private int cacheObjectMaxSize = 512; // 512 KB

	/**
	 * Non cacheable resources.
	 */
	private String[] nonCacheable = { "/WEB-INF/lib/", "/WEB-INF/classes/" };
	...
	protected boolean validate(CacheEntry entry) {
		if (((!entry.isExists()) || (entry.getContext() != null) || ((entry
				.getResource() != null) && (entry.getResource().getContent() != null)))
	...
	protected boolean revalidate(CacheEntry entry) {
		...
		if (lastModified <= 0)
			return false;
		try {
			Attributes tempAttributes = dirContext.getAttributes(entry
					.getName());
	...
	protected void cacheLoad(CacheEntry entry) {
		...
		if ((exists)
				...
				&& (entry.getAttributes().getContentLength() < (cacheObjectMaxSize * 1024))) {
			...
			entry.setSize(entry.getSize()
					+ ((int) entry.getAttributes().getContentLength() / 1024));
			InputStream is = null;
			try {
		...
		// Set timestamp
		entry.setTimestamp(System.currentTimeMillis() + cacheTTL);

		// Add new entry to cache
		synchronized (cache) {
			// Check cache size, and remove elements if too big
			if ((cache.lookup(name) == null) && cache.allocate(entry.getSize())) {
				cache.load(entry);
			}
		}
	...
	public ProxyDirContext getProxy() {
		return proxy;
	}

	public void setProxy(ProxyDirContext proxy) {
		this.proxy = proxy;
	}
	...
	public String[] getNonCacheable() {
		return nonCacheable;
	}

	public void setNonCacheable(String[] nonCacheable) {
	...
	public static NameNotFoundException getNotFoundException() {
