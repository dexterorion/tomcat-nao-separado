...
public class ServiceRefFactory implements ObjectFactory {
	...
	public Object getObjectInstance(Object obj, Name name, Context nameCtx,
		...
		if (obj instanceof ServiceRef) {
			...
			// Service Interface
			RefAddr tmp = ref.get(ServiceRef.getServiceInterface());
			String serviceInterface = null;
			if (tmp != null)
				serviceInterface = (String) tmp.getContent();

			// WSDL
			tmp = ref.get(ServiceRef.getWsdl());
			...
			tmp = ref.get(ServiceRef.getServiceLocalPart());
			if (tmp != null) {
				String serviceLocalPart = (String) tmp.getContent();
				tmp = ref.get(ServiceRef.getServiceNamespace());
				if (tmp == null) {
			...
			for (int i = 0; i < ref.size(); i++)
				if (ServiceRef.getServiceendpointinterface().equals(ref.get(i)
						.getType())) {
					String serviceendpoint = "";
					String portlink = "";
					serviceendpoint = (String) ref.get(i).getContent();
					if (ServiceRef.getPortcomponentlink().equals(ref.get(i + 1)
							.getType())) {
						i++;
			...
			if (((ServiceRef) ref).getHandlersSize() > 0) {
				...
				while (((ServiceRef) ref).getHandlersSize() > 0) {
					...
					tmp = handlerRef.get(HandlerRef.getHandlerClass());
					...
					for (int i = 0; i < handlerRef.size(); i++)
						if (HandlerRef.getHandlerLocalpart().equals(handlerRef.get(
								i).getType())) {
							String localpart = "";
							String namespace = "";
							localpart = (String) handlerRef.get(i).getContent();
							if (HandlerRef.getHandlerNamespace().equals(handlerRef
									.get(i + 1).getType())) {
								i++;
							...
							headers.add(header);
						} else if (HandlerRef.getHandlerParamname()
								.equals(handlerRef.get(i).getType())) {
							String paramName = "";
							String paramValue = "";
							paramName = (String) handlerRef.get(i).getContent();
							if (HandlerRef.getHandlerParamvalue().equals(handlerRef
									.get(i + 1).getType())) {
								i++;
						...
						} else if (HandlerRef.getHandlerSoaprole()
								.equals(handlerRef.get(i).getType())) {
							String soaprole = "";
							soaprole = (String) handlerRef.get(i).getContent();
							soaproles.add(soaprole);
						} else if (HandlerRef.getHandlerPortname()
								.equals(handlerRef.get(i).getType())) {
							String portName = "";
