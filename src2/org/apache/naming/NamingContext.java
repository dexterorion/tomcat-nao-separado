...
public class NamingContext implements Context {
     ...
     */
    private static final StringManager2 sm = StringManager2.getManager(Constants29.getPackage());


    /**
     * Bindings in this Context.
     */
    private HashMap<String,NamingEntry> bindings;
    ...
    public void unbind(Name name) throws NamingException {
        ...
        if (name.size() > 1) {
            if (entry.getType() == NamingEntry.getContext()) {
                ((Context) entry.getValue()).unbind(name.getSuffix(1));
            } else {
                throw new NamingException
    ...
    public NamingEnumeration<NameClassPair> list(Name name)
        ...
        return ((Context) entry.getValue()).list(name.getSuffix(1));
    }
    ...
    public NamingEnumeration<Binding> listBindings(Name name)
        ...
        return ((Context) entry.getValue()).listBindings(name.getSuffix(1));
    }
    ...
    public void destroySubcontext(Name name) throws NamingException {
        ...
        if (name.size() > 1) {
            if (entry.getType() == NamingEntry.getContext()) {
                ((Context) entry.getValue()).destroySubcontext(name.getSuffix(1));
            } else {
                throw new NamingException
                    (sm.getString("namingContext.contextExpected"));
            }
    ...
    protected Object lookup(Name name, boolean resolveLinks)
        ...
        if (name.size() > 1) {
            ...
            return ((Context) entry.getValue()).lookup(name.getSuffix(1));
        } else {
            if ((resolveLinks) && (entry.getType() == NamingEntry.getLinkRef())) {
                String link = ((LinkRef) entry.getValue()).getLinkName();
                if (link.startsWith(".")) {
            ...
            } else if (entry.getType() == NamingEntry.getReference()) {
                try {
                    Object obj = NamingManager.getObjectInstance
                        (entry.getValue(), name, this, env);
                    if(entry.getValue() instanceof ResourceRef) {
                        boolean singleton = Boolean.parseBoolean(
                                    (String) ((ResourceRef) entry.getValue()).get(
                                        "singleton").getContent());
                        if (singleton) {
                            entry.setType(NamingEntry.getEntry());
                            entry.setValue(obj);
                        }
                    }
                    return obj; 
    ...
    protected void bind(Name name, Object obj, boolean rebind)
        ...
        if (name.size() > 1) {
            ...
            if (entry.getType() == NamingEntry.getContext()) {
                if (rebind) {
                    ((Context) entry.getValue()).rebind(name.getSuffix(1), obj);
                } else {
                    ((Context) entry.getValue()).bind(name.getSuffix(1), obj);
                }
            } else {
                throw new NamingException
                    (sm.getString("namingContext.contextExpected"));
            ...
            if ((!rebind) && (entry != null)) {
                ...
                if (toBind instanceof Context) {
                    entry = new NamingEntry(name.get(0), toBind, 
                                            NamingEntry.getContext());
                } else if (toBind instanceof LinkRef) {
                    entry = new NamingEntry(name.get(0), toBind, 
                                            NamingEntry.getLinkRef());
                } else if (toBind instanceof Reference) {
                    entry = new NamingEntry(name.get(0), toBind, 
                                            NamingEntry.getReference());
