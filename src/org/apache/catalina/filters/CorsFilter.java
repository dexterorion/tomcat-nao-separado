...
public final class CorsFilter implements Filter {
	...
	private static final StringManager3 sm = StringManager3.getManager(Constants5
			.getPackage());
	...
	public void doFilter(final ServletRequest servletRequest,
		...
		CorsFilterCORSRequestType requestType = checkRequestType(request);
	...
	/**
	 * Handles a CORS request of type {@link CorsFilterCORSRequestType}.SIMPLE.
	 *
	 * @param request
	...
	protected void handleSimpleCORS(final HttpServletRequest request,
		...
		CorsFilterCORSRequestType requestType = checkRequestType(request);
		if (!(requestType == CorsFilterCORSRequestType.SIMPLE || requestType == CorsFilterCORSRequestType.ACTUAL)) {
			throw new IllegalArgumentException(sm.getString(
					"corsFilter.wrongType2", CorsFilterCORSRequestType.SIMPLE,
					CorsFilterCORSRequestType.ACTUAL));
		}
	...
	protected void handlePreflightCORS(final HttpServletRequest request,
		...
		CorsFilterCORSRequestType requestType = checkRequestType(request);
		if (requestType != CorsFilterCORSRequestType.PRE_FLIGHT) {
			throw new IllegalArgumentException(sm.getString(
					"corsFilter.wrongType1",
					CorsFilterCORSRequestType.PRE_FLIGHT.name().toLowerCase()));
		}
	 ...
	 *            The {@link CorsFilterCORSRequestType} object.
	 */
	protected static void decorateCORSProperties(
			final HttpServletRequest request,
			final CorsFilterCORSRequestType corsRequestType) {
		if (request == null) {
	 ...
	 * @param request
	 */
	protected CorsFilterCORSRequestType checkRequestType(
			final HttpServletRequest request) {
		CorsFilterCORSRequestType requestType = CorsFilterCORSRequestType.INVALID_CORS;
		if (request == null) {
			throw new IllegalArgumentException(
					sm.getString("corsFilter.nullRequest"));
		}
		...
		if (originHeader != null) {
			if (originHeader.isEmpty()) {
				requestType = CorsFilterCORSRequestType.INVALID_CORS;
				...
				if (method != null && HTTP_METHODS.contains(method)) {
					if ("OPTIONS".equals(method)) {
						...
						if (accessControlRequestMethodHeader != null
							...
							requestType = CorsFilterCORSRequestType.PRE_FLIGHT;
						} else if (accessControlRequestMethodHeader != null
								&& accessControlRequestMethodHeader.isEmpty()) {
							requestType = CorsFilterCORSRequestType.INVALID_CORS;
						} else {
							requestType = CorsFilterCORSRequestType.ACTUAL;
						}
					} else if ("GET".equals(method) || "HEAD".equals(method)) {
						requestType = CorsFilterCORSRequestType.SIMPLE;
			...
			requestType = CorsFilterCORSRequestType.NOT_CORS;
		}
	 ...
	 */
	private static final String HTTP_REQUEST_ATTRIBUTE_ORIGIN = HTTP_REQUEST_ATTRIBUTE_PREFIX
			+ "request.origin";

	/**
	 * Boolean value, suggesting if the request is a CORS request or not.
	 */
	private static final String HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST = HTTP_REQUEST_ATTRIBUTE_PREFIX
			+ "isCorsRequest";

	/**
	 * Type of CORS request, of type {@link CorsFilterCORSRequestType}.
	 */
	private static final String HTTP_REQUEST_ATTRIBUTE_REQUEST_TYPE = HTTP_REQUEST_ATTRIBUTE_PREFIX
			+ "request.type";
	 ...
	 * pre-flight request.
	 */
	private static final String HTTP_REQUEST_ATTRIBUTE_REQUEST_HEADERS = HTTP_REQUEST_ATTRIBUTE_PREFIX
			+ "request.headers";
	 ...
	 *
	 */
	private static final Collection<String> HTTP_METHODS = new HashSet<String>(
			Arrays.asList("OPTIONS", "GET", "HEAD", "POST", "PUT", "DELETE",
					"TRACE", "CONNECT"));
	/**
	 * {@link Collection} of non-simple HTTP methods. Case sensitive.
	 */
	private static final Collection<String> COMPLEX_HTTP_METHODS = new HashSet<String>(
	 ...
	 */
	private static final Collection<String> SIMPLE_HTTP_METHODS = new HashSet<String>(
	 ...
	 *      >http://www.w3.org/TR/cors/#terminology</a>
	 */
	private static final Collection<String> SIMPLE_HTTP_REQUEST_HEADERS = new HashSet<String>(
	 ...
	 *      >http://www.w3.org/TR/cors/#terminology</a>
	 */
	private static final Collection<String> SIMPLE_HTTP_RESPONSE_HEADERS = new HashSet<String>(
	 ...
	 *      >http://www.w3.org/TR/cors/#terminology</a>
	 */
	private static final Collection<String> SIMPLE_HTTP_REQUEST_CONTENT_TYPE_VALUES = new HashSet<String>(
	 ...
	 */
	private static final String DEFAULT_ALLOWED_HTTP_METHODS = "GET,POST,HEAD,OPTIONS";

	/**
	 * By default, time duration to cache pre-flight response is 30 mins.
	 */
	private static final String DEFAULT_PREFLIGHT_MAXAGE = "1800";

	/**
	 * By default, support credentials is turned on.
	 */
	private static final String DEFAULT_SUPPORTS_CREDENTIALS = "true";
	 ...
	 */
	private static final String DEFAULT_ALLOWED_HTTP_HEADERS = "Origin,Accept,X-Requested-With,Content-Type,"
	 ...
	 */
	private static final String DEFAULT_DECORATE_REQUEST = "true";

	// ----------------------------------------Filter Config Init param-name(s)
	/**
	 * Key to retrieve allowed origins from {@link FilterConfig}.
	 */
	private static final String PARAM_CORS_ALLOWED_ORIGINS = "cors.allowed.origins";

	/**
	 * Key to retrieve support credentials from {@link FilterConfig}.
	 */
	private static final String PARAM_CORS_SUPPORT_CREDENTIALS = "cors.support.credentials";

	/**
	 * Key to retrieve exposed headers from {@link FilterConfig}.
	 */
	private static final String PARAM_CORS_EXPOSED_HEADERS = "cors.exposed.headers";

	/**
	 * Key to retrieve allowed headers from {@link FilterConfig}.
	 */
	private static final String PARAM_CORS_ALLOWED_HEADERS = "cors.allowed.headers";

	/**
	 * Key to retrieve allowed methods from {@link FilterConfig}.
	 */
	private static final String PARAM_CORS_ALLOWED_METHODS = "cors.allowed.methods";

	/**
	 * Key to retrieve preflight max age from {@link FilterConfig}.
	 */
	private static final String PARAM_CORS_PREFLIGHT_MAXAGE = "cors.preflight.maxage";

	/**
	 * Key to determine if request should be decorated.
	 */
	private static final String PARAM_CORS_REQUEST_DECORATE = "cors.request.decorate";
}
