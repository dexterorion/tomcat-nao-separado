...
public class SingleSignOn extends ValveBase implements SessionListener {
     ...
     */
    private Map<String,SingleSignOnEntry> cache =
     ...
     */
    private Map<Session2,String> reverse = new HashMap<Session2,String>();
    ...
    private static final StringManager3 sm =
        StringManager3.getManager(Constants.getPackage());
    ...
    public void sessionEvent(SessionEvent event) {
        ...
        if (!Session2.SESSION_DESTROYED_EVENT.equals(event.getType())
                && (!Session2.SESSION_PASSIVATED_EVENT.equals(event.getType())))
            return;

        // Look up the single session id associated with this session (if any)
        Session2 session = event.getSession();
        if (getContainerLog().isDebugEnabled())
            getContainerLog().debug("Process session destroyed on " + session);
    ...
    public void invoke(Request request, Response response)
        ...
        request.removeNote(Constants.getReqSsoidNote());

        // Has a valid user already been authenticated?
        if (getContainerLog().isDebugEnabled())
            getContainerLog().debug("Process request for '" + request.getRequestURI() + "'");
        if (request.getUserPrincipal() != null) {
            if (getContainerLog().isDebugEnabled())
                getContainerLog().debug(" Principal '" + request.getUserPrincipal().getName() +
        ...
        if (getContainerLog().isDebugEnabled())
            getContainerLog().debug(" Checking for SSO cookie");
        Cookie cookie = null;
        Cookie cookies[] = request.getCookies();
        if (cookies == null)
            cookies = new Cookie[0];
        for (int i = 0; i < cookies.length; i++) {
            if (Constants.getSingleSignOnCookie().equals(cookies[i].getName())) {
                cookie = cookies[i];
                break;
            }
        }
        if (cookie == null) {
            if (getContainerLog().isDebugEnabled())
                getContainerLog().debug(" SSO cookie is not present");
            getNext().invoke(request, response);
            return;
        }
        ...
        if (getContainerLog().isDebugEnabled())
            getContainerLog().debug(" Checking for cached principal for " + cookie.getValue());
        SingleSignOnEntry entry = lookup(cookie.getValue());
        if (entry != null) {
            if (getContainerLog().isDebugEnabled())
                getContainerLog().debug(" Found cached principal '" +
                    (entry.getPrincipal() != null ? entry.getPrincipal().getName() : "") + "' with auth type '" +
                    entry.getAuthType() + "'");
            request.setNote(Constants.getReqSsoidNote(), cookie.getValue());
            ...
            if (getContainerLog().isDebugEnabled())
                getContainerLog().debug(" No cached principal found, erasing SSO cookie");
     ...
     */
    protected void associate(String ssoId, Session2 session) {

        if (getContainerLog().isDebugEnabled())
            getContainerLog().debug("Associate sso id " + ssoId + " with session " + session);
     ...
     */
    protected void deregister(String ssoId, Session2 session) {
        ...
        Session2 sessions[] = sso.findSessions();
    ...
    protected void deregister(String ssoId) {
        ...
        if (getContainerLog().isDebugEnabled())
            getContainerLog().debug("Deregistering sso id '" + ssoId + "'");
        ...
        for (int i = 0; i < sessions.length; i++) {
            if (getContainerLog().isTraceEnabled())
                getContainerLog().trace(" Invalidating session " + sessions[i]);
    ...
    protected void register(String ssoId, Principal principal, String authType,
        ...
        if (getContainerLog().isDebugEnabled())
            getContainerLog().debug("Registering sso id '" + ssoId + "' for user '" +
    ...
    protected void update(String ssoId, Principal principal, String authType,
        ...
        if (sso != null && !sso.getCanReauthenticate()) {
            if (getContainerLog().isDebugEnabled())
                getContainerLog().debug("Update sso id " + ssoId + " to auth type " + authType);
     ...
     */
    protected void removeSession(String ssoId, Session2 session) {

        if (getContainerLog().isDebugEnabled())
            getContainerLog().debug("Removing session " + session.toString() + " from sso id " + 
                ssoId );
