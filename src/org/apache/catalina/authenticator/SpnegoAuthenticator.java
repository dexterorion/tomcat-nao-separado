...
public class SpnegoAuthenticator extends AuthenticatorBase {
    ...
    private String loginConfigName = Constants.getDefaultLoginModuleName();
    ...
    protected String getAuthMethod() {
        return Constants.getSpnegoMethod();
    }
    ...
    protected void initInternal() throws LifecycleException {
        ...
        String krb5Conf = System.getProperty(Constants.getKrb5ConfProperty());
        if (krb5Conf == null) {
            // System property not set, use the Tomcat default
            File krb5ConfFile = new File(Bootstrap.getCatalinaBase(),
                    Constants.getDefaultKrb5Conf());
            System.setProperty(Constants.getKrb5ConfProperty(),
                    krb5ConfFile.getAbsolutePath());
        }

        // JAAS configuration file location
        String jaasConf = System.getProperty(Constants.getJaasConfProperty());
        if (jaasConf == null) {
            // System property not set, use the Tomcat default
            File jaasConfFile = new File(Bootstrap.getCatalinaBase(),
                    Constants.getDefaultJaasConf());
            System.setProperty(Constants.getJaasConfProperty(),
    ...
    public boolean authenticate(Request request, HttpServletResponse response,
        ...
        if (authorization == null) {
            if (log.isDebugEnabled()) {
                log.debug(getSm().getString("authenticator.noAuthHeader"));
            }
        ...
        byte[] decoded = Base642.decodeBase64(authorizationBC.getBuffer(),
        ...
        try {
            try {
                ...
                log.error(getSm().getString("spnegoAuthenticator.serviceLoginFail"),
                        e);
                response.sendError(
            ...
            outToken = Subject.doAs(lc.getSubject(), new SpnegoAuthenticatorAcceptAction(gssContext, decoded));
            ...
            principal = Subject.doAs(subject, new SpnegoAuthenticatorAuthenticateAction(
                    getContext().getRealm(), gssContext, storeDelegatedCredential));

        } catch (GSSException e) {
            if (log.isDebugEnabled()) {
                log.debug(getSm().getString("spnegoAuthenticator.ticketValidateFail"), e);
            }
            response.setHeader("WWW-Authenticate", "Negotiate");
            response.sendError(HttpServletResponse.SC_UNAUTHORIZED);
            return false;
            ...
            if (cause instanceof GSSException) {
                if (log.isDebugEnabled()) {
                    log.debug(getSm().getString("spnegoAuthenticator.serviceLoginFail"), e);
                }
            } else {
                log.error(getSm().getString("spnegoAuthenticator.serviceLoginFail"), e);
            }
            response.setHeader("WWW-Authenticate", "Negotiate");
            response.sendError(HttpServletResponse.SC_UNAUTHORIZED);
            return false;
        ...
        response.setHeader("WWW-Authenticate", "Negotiate "
                + Base642.encodeBase64String(outToken));

        if (principal != null) {
            register(request, response, principal, Constants.getSpnegoMethod(),
