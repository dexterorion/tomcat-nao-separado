	...
	public boolean authenticate(Request request, HttpServletResponse response,
		...
		if (principal != null) {
			...
			if (getCache()) {
				...
				Session2 session = request.getSessionInternal(true);
				...
				String ssoId = (String) request.getNote(Constants
						.getReqSsoidNote());
				if (ssoId != null) {
					if (getContainerLog().isDebugEnabled())
						getContainerLog().debug(
								"User authenticated by existing SSO");
		...
		if (getContainerLog().isDebugEnabled())
			getContainerLog().debug("User authenticated without any roles");
		return true;
	}
