...
public class JAASRealm extends RealmBase {
     ...
     * Descriptive information about this <code>Realm</code> implementation.
     */
    private static final String name = "JAASRealm";


    /**
     * The list of role class names, split out for easy processing.
     */
    private List<String> roleClasses = new ArrayList<String>();


    /**
     * The set of user class names, split out for easy processing.
     */
    private List<String> userClasses = new ArrayList<String>();
    ...
    private volatile boolean jaasConfigurationLoaded = false;
    ...
    public void setContainer(Container container) {
      ...
      */
    private String roleClassNames = null;
      ...
      */
     private String userClassNames = null;
    ...
    protected Principal authenticate(String username,
        ...
        if( log.isDebugEnabled())
            log.debug(getSm().getString("jaasRealm.beginLogin", username, appName));
        ...
        try {
            ...
            log.error(getSm().getString("jaasRealm.unexpectedError"), e);
            return (null);
        } finally {
            if(!isUseContextClassLoader()) {
        ...
        try {
            ...
            if (subject == null) {
                if( log.isDebugEnabled())
                    log.debug(getSm().getString("jaasRealm.failedLogin", username));
                return (null);
            }
        } catch (AccountExpiredException e) {
            if (log.isDebugEnabled())
                log.debug(getSm().getString("jaasRealm.accountExpired", username));
            return (null);
        } catch (CredentialExpiredException e) {
            if (log.isDebugEnabled())
                log.debug(getSm().getString("jaasRealm.credentialExpired", username));
            return (null);
            ...
            log.warn(getSm().getString("jaasRealm.loginException", username), e);
            return (null);
        } catch (Throwable e) {
            ExceptionUtils2.handleThrowable(e);
            log.error(getSm().getString("jaasRealm.unexpectedError"), e);
        ...
        if( log.isDebugEnabled())
            log.debug(getSm().getString("jaasRealm.loginContextCreated", username));

        // Return the appropriate Principal for this authenticated Subject
        Principal principal = createPrincipal(username, subject, loginContext);
        if (principal == null) {
            log.debug(getSm().getString("jaasRealm.authenticateFailure", username));
            return (null);
        }
        if (log.isDebugEnabled()) {
            log.debug(getSm().getString("jaasRealm.authenticateSuccess", username));
        }
    ...
    protected Principal createPrincipal(String username, Subject subject,
        ...
        while (principals.hasNext()) {
            ...
            if( log.isDebugEnabled() ) {
                log.debug(getSm().getString("jaasRealm.checkPrincipal", principal, principalClass));
            }

            if (userPrincipal == null && userClasses.contains(principalClass)) {
                userPrincipal = principal;
                if( log.isDebugEnabled() ) {
                    log.debug(getSm().getString("jaasRealm.userPrincipalSuccess", principal.getName()));
                }
            }
            ...
            if (roleClasses.contains(principalClass)) {
                ...
                if( log.isDebugEnabled() ) {
                    log.debug(getSm().getString("jaasRealm.rolePrincipalAdd", principal.getName()));
                }
            }
        }
        ...
        if (userPrincipal == null) {
            if (log.isDebugEnabled()) {
                log.debug(getSm().getString("jaasRealm.userPrincipalFailure"));
                log.debug(getSm().getString("jaasRealm.rolePrincipalFailure"));
            }
        } else {
            if (roles.size() == 0) {
                if (log.isDebugEnabled()) {
