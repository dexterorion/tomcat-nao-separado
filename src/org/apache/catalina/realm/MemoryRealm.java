...
public class MemoryRealm  extends RealmBase {
    ...
    private static final String name = "MemoryRealm";
    ...
    public Principal authenticate(String username, String credentials) {
        ...
        if (validated) {
            if (log.isDebugEnabled())
                log.debug(getSm().getString("memoryRealm.authenticateSuccess", username));
            return (principal);
        } else {
            if (log.isDebugEnabled())
                log.debug(getSm().getString("memoryRealm.authenticateFailure", username));
            return (null);
        }
    ...
    protected synchronized Digester getDigester() {
        ...
        if (digester == null) {
            ...
            try {
                ...
                log.warn(getSm().getString("memoryRealm.xmlFeatureEncoding"), e);
            }
    ...
    protected void startInternal() throws LifecycleException {
        ...
        if (!file.isAbsolute())
            file = new File(System.getProperty(Globals.getCatalinaBaseProp()), pathname);
        if (!file.exists() || !file.canRead())
            throw new LifecycleException
                (getSm().getString("memoryRealm.loadExist",
                              file.getAbsolutePath()));

        // Load the contents of the database file
        if (log.isDebugEnabled())
            log.debug(getSm().getString("memoryRealm.loadPath",
                             file.getAbsolutePath()));
        ...
        try {
            ...
            throw new LifecycleException
                (getSm().getString("memoryRealm.readXml"), e);
        } finally {
            digester.reset();
        }
