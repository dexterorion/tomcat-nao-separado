...
public class JNDIRealm extends RealmBase {
     ...
     * Descriptive information about this Realm implementation.
     */
	private static final String name = "JNDIRealm";
     ...
     */
	private boolean adCompat = false;
     ...
     * available
     */
	private boolean useDelegatedCredential = true;
     ...
     * connection.
     */
	private String spnegoDelegationQop = "auth-conf";
        ...
        try {
            ...
            try {
                ...
                */
                ...
                getContainerLog().info(getSm().getString("jndiRealm.exception.retry"), e);
            ...
            getContainerLog().error(getSm().getString("jndiRealm.exception"), e);
            ...
            if (getContainerLog().isDebugEnabled())
                getContainerLog().debug("Returning null principal.");
            return (null);
    ...
    public synchronized Principal authenticate(DirContext context,
        ...
        if (username == null || username.equals("")
            ...
            if (getContainerLog().isDebugEnabled())
                getContainerLog().debug("username null or empty: returning null principal.");
            return (null);
        }
        ...
        if (userPatternArray != null) {
            for (int curUserPattern = 0;
            	...
            	JNDIRealmUser user = getUser(context, username, credentials, curUserPattern);
                if (user != null) {
                    try {
                        ...
                        if (checkCredentials(context, user, credentials)) {
                            ...
                            if (getContainerLog().isDebugEnabled()) {
                                ...
                                while (it.hasNext()) {
                                    getContainerLog().debug("Found role: " + it.next());
                                }
                            }
                        ...
                        // Log the problem for posterity
                        getContainerLog().warn(getSm().getString("jndiRealm.exception"), ine);
            ...
            // Retrieve user information
        	JNDIRealmUser user = getUser(context, username, credentials);
            if (user == null)
                return (null);
     ...
     */
    protected JNDIRealmUser getUser(DirContext context, String username)
        throws NamingException {
     ...
     * @see #getUser(DirContext, String, String, int)
     */
    protected JNDIRealmUser getUser(DirContext context, String username, String credentials)
        throws NamingException {
    ...
    protected JNDIRealmUser getUser(DirContext context, String username,
        ...
        if (userPassword == null && credentials != null) {
            ...
            // role searches.
            return new JNDIRealmUser(user.getUserName(), user.getDN(), credentials,
     ...
     * @exception NamingException if a directory server error occurs
     */
    protected JNDIRealmUser getUserByPattern(DirContext context,
                                    String username,
                                    String[] attrIds,
                                    String dn)
        throws NamingException {
        ...
        if (attrIds == null || attrIds.length == 0) {
            return new JNDIRealmUser(username, dn, null, null,null);
        }
        ...
        return new JNDIRealmUser(username, dn, password, roles, userRoleAttrValue);
    }
     ...
     * @exception NamingException if a directory server error occurs
     */
    protected JNDIRealmUser getUserBySearch(DirContext context,
                                   String username,
                                   String[] attrIds)
        throws NamingException {
        ...
        try {
            if (results.hasMore()) {
                if(getContainerLog().isInfoEnabled())
                    getContainerLog().info("username " + username + " has multiple entries");
                return (null);
            }
        } catch (PartialResultException ex) {
            if (!adCompat)
        ...
        if (getContainerLog().isTraceEnabled())
            getContainerLog().trace("  entry found for " + username + " with dn " + dn);
    ...
    protected boolean checkCredentials(DirContext context,
         ...
         if (getContainerLog().isTraceEnabled()) {
             if (validated) {
                 getContainerLog().trace(getSm().getString("jndiRealm.authenticateSuccess",
                                  user.getUserName()));
             } else {
                 getContainerLog().trace(getSm().getString("jndiRealm.authenticateFailure",
                                  user.getUserName()));
             }
         }
         return (validated);
    ...
    protected boolean compareCredentials(DirContext context,
        ...
        if (getContainerLog().isTraceEnabled())
            getContainerLog().trace("  validating credentials");
     ...
     */
     protected boolean bindAsUser(DirContext context,
         ...
         // Validate the credentials specified by the user
         if (getContainerLog().isTraceEnabled()) {
             getContainerLog().trace("  validating credentials by binding as the user");
        }
        ...
        try {
            if (getContainerLog().isTraceEnabled()) {
                getContainerLog().trace("  binding as "  + dn);
            }
            ...
            if (getContainerLog().isTraceEnabled()) {
                getContainerLog().trace("  bind attempt failed");
            }
        }
     ...
     * @exception NamingException if a directory server error occurs
     */
    protected List<String> getRoles(DirContext context, JNDIRealmUser user)
        throws NamingException {
        ...
        if (getContainerLog().isTraceEnabled())
            getContainerLog().trace("  getRoles(" + dn + ")");
        ...
        if (getContainerLog().isTraceEnabled()) {
            getContainerLog().trace("  Found " + list.size() + " user internal roles");
            for (int i=0; i<list.size(); i++)
                getContainerLog().trace(  "  Found user internal role " + list.get(i));
        }
        ...
        if (getContainerLog().isTraceEnabled()) {
            getContainerLog().trace("  Found " + keys.size() + " direct roles");
            for (String key: keys) {
                getContainerLog().trace(  "  Found direct role " + key + " -> " + groupMap.get(key));
            }
        }
        ...
        if (getRoleNested()) {
            ...
            while (!newGroups.isEmpty()) {
                ...
                for (Entry<String, String> group : newGroups.entrySet()) {
                    ...
                    if (getContainerLog().isTraceEnabled()) {
                        getContainerLog().trace("Perform a nested group search with base "+ roleBase + " and filter " + filter);
                    }
                    ...
                    try {
                        while (results.hasMore()) {
                            ...
                            if (name != null && dname != null && !groupMap.keySet().contains(dname)) {
                                ...
                                if (getContainerLog().isTraceEnabled()) {
                                    getContainerLog().trace("  Found nested role " + dname + " -> " + name);
                                }
    ...
    private String getAttributeValue(String attrId, Attributes attrs)
        ...
        if (getContainerLog().isTraceEnabled())
            getContainerLog().trace("  retrieving attribute " + attrId);
    ...
    private ArrayList<String> addAttributeValues(String attrId,
        ...
        if (getContainerLog().isTraceEnabled())
            getContainerLog().trace("  retrieving values for attribute " + attrId);
        if (attrId == null || attrs == null)
            return values;
        if (values == null)
    ...
    protected void close(DirContext context) {
        ...
        try {
            if (getContainerLog().isDebugEnabled())
                getContainerLog().debug("Closing directory context");
            context.close();
        } catch (NamingException e) {
            getContainerLog().error(getSm().getString("jndiRealm.close"), e);
        }
        this.context = null;
    ...
    protected Hashtable<String,String> getDirectoryContextEnvironment() {
        ...
        if (getContainerLog().isDebugEnabled() && connectionAttempt == 0)
            getContainerLog().debug("Connecting to URL " + connectionURL);
        else if (getContainerLog().isDebugEnabled() && connectionAttempt > 0)
            getContainerLog().debug("Connecting to URL " + alternateURL);
    ...
    protected void startInternal() throws LifecycleException {
        ...
        try {
            ...
            getContainerLog().error(getSm().getString("jndiRealm.open"), e);
        }
    ...
    protected String getDistinguishedName(DirContext context, String base,
        ...
        if (result.isRelative()) {
           if (getContainerLog().isTraceEnabled()) {
               getContainerLog().trace("  search returned relative name: " +
                       result.getName());
           }
           ...
           if (getContainerLog().isTraceEnabled())
               getContainerLog().trace("  search returned absolute name: " +
                       result.getName());
           try {
