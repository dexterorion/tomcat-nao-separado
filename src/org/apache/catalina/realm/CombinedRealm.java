...
public class CombinedRealm extends RealmBase {
     ...
     */
    private List<Realm> realms = new LinkedList<Realm>();

    /**
     * Descriptive information about this Realm implementation.
     */
    private static final String name = "CombinedRealm";
    ...
    public void addRealm(Realm theRealm) {
        ...
        if (log.isDebugEnabled()) {
            getSm().getString("combinedRealm.addRealm", theRealm.getInfo(), 
    ...
    public Principal authenticate(String username, String clientDigest,
        ...
        for (Realm realm : realms) {
            if (log.isDebugEnabled()) {
                log.debug(getSm().getString("combinedRealm.authStart", username, realm.getInfo()));
            }
            ...
            if (authenticatedUser == null) {
                if (log.isDebugEnabled()) {
                    log.debug(getSm().getString("combinedRealm.authFail", username, realm.getInfo()));
                }
            } else {
                if (log.isDebugEnabled()) {
                    log.debug(getSm().getString("combinedRealm.authSuccess", username, realm.getInfo()));
                }
                break;
            }
        }
        ...
        while (iter.hasNext()) {
            ...
            if (realm instanceof Lifecycle) {
                try {
                    ...
                    iter.remove();
                    log.error(getSm().getString("combinedRealm.realmStartFail",
                            realm.getInfo()), e);
                }
            }
        }
    ...
    public Principal authenticate(GSSContext gssContext, boolean storeCreds) {
        if (gssContext.isEstablished()) {
            ...
            try {
                ...
                log.warn(getSm().getString("realmBase.gssNameFail"), e);
                return null;
            }
            ...
            for (Realm realm : realms) {
                if (log.isDebugEnabled()) {
                    log.debug(getSm().getString("combinedRealm.authStart",
                ...
                if (authenticatedUser == null) {
                    if (log.isDebugEnabled()) {
                        log.debug(getSm().getString("combinedRealm.authFail",
                                username, realm.getInfo()));
                    }
                } else {
                    if (log.isDebugEnabled()) {
                        log.debug(getSm().getString("combinedRealm.authSuccess",
                                username, realm.getInfo()));
                    }
                    break;
                }
    ...
    protected String getPassword(String username) {
        ...
        UnsupportedOperationException uoe =
            new UnsupportedOperationException(
                    getSm().getString("combinedRealm.getPassword"));
        log.error(getSm().getString("combinedRealm.unexpectedMethod"), uoe);
        throw uoe;
    }
    ...
    protected Principal getPrincipal(String username) {
        ...
        UnsupportedOperationException uoe =
            new UnsupportedOperationException(
                    getSm().getString("combinedRealm.getPrincipal"));
        log.error(getSm().getString("combinedRealm.unexpectedMethod"), uoe);
        throw uoe;
    }
