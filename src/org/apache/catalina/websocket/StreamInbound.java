...
public abstract class StreamInbound implements UpgradeInbound {
    ...
    private int outboundByteBufferSize = WsOutbound.getDefaultBufferSize();
    private int outboundCharBufferSize = WsOutbound.getDefaultBufferSize();
    ...
    public final SocketState onData() throws IOException {
        ...
        try {
            ...
            while (frame != null) {
                ...
                if (frame.getRsv() > 0) {
                    closeOutboundConnection(
                            Constants23.getStatusProtocolError(), null);
                    return SocketState.CLOSED;
                }

                byte opCode = frame.getOpCode();

                if (opCode == Constants23.getOpcodeBinary()) {
                    doOnBinaryData(wsIs);
                } else if (opCode == Constants23.getOpcodeText()) {
                    InputStreamReader r =
                            new InputStreamReader(wsIs, new Utf8Decoder());
                    doOnTextData(r);
                } else if (opCode == Constants23.getOpcodeClose()){
                    closeOutboundConnection(frame);
                    return SocketState.CLOSED;
                } else if (opCode == Constants23.getOpcodePing()) {
                    getWsOutbound().pong(frame.getPayLoad());
                } else if (opCode == Constants23.getOpcodePong()) {
            ...
            // Invalid UTF-8
            closeOutboundConnection(Constants23.getStatusBadData(), null);
            return SocketState.CLOSED;
            ...
            closeOutboundConnection(Constants23.getStatusProtocolError(), null);
            return SocketState.CLOSED;
        }
    ...
    private void closeOutboundConnection(WsFrame frame) throws IOException {
        try {
            ...
            doOnClose(Constants23.getStatusCloseNormal());
        }
    }
     ...
     */
    public void doOnClose(int status) {
