...
public class TcpSender implements Sender {
	...
	private HeartbeatListener config = null;

	/**
	 * Proxies.
	 */
	private Proxy[] proxies = null;
	...
	private BufferedReader[] connectionReaders = null;
	private BufferedWriter[] connectionWriters = null;
	...
	public void init(HeartbeatListener config) throws Exception {
		...
		while (tok.hasMoreTokens()) {
			...
			proxies[i] = new Proxy();
			proxies[i].setPort(Integer.parseInt(token.substring(pos + 1)));
			try {
				proxies[i].setAddress(InetAddress.getByName(token.substring(0,
						pos)));
			} catch (Exception e) {
				throw new Exception("bad ProxyList");
			}
			...
			if (connections[i] == null) {
				try {
					if (config.getHost() != null) {
						...
						addrs = new InetSocketAddress(proxies[i].getAddress(),
								proxies[i].getPort());
						connections[i].connect(addrs);
					} else
						connections[i] = new Socket(proxies[i].getAddress(),
								proxies[i].getPort());
