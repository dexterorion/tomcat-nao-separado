...
public class WebappLoader extends LifecycleMBeanBase implements Loader,
	 ...
	 */
	private static final StringManager3 sm = StringManager3.getManager(Constants9
			.getPackage());
	...
	protected void startInternal() throws LifecycleException {
		...
		try {
			...
			Registry2.getRegistry(null, null).registerComponent(classLoader,
					cloname, null);
	...
	protected void stopInternal() throws LifecycleException {
		...
		if (container instanceof Context) {
			ServletContext servletContext = ((Context) container)
					.getServletContext();
			servletContext.removeAttribute(Globals.getClassPathAttr());
		}
		...
		try {
			...
			ObjectName cloname = new ObjectName(MBeanUtils.getDomain(ctx)
					+ ":type=WebappClassLoader,context=" + contextName
					+ ",host=" + ctx.getParent().getName());
			Registry2.getRegistry(null, null).unregisterComponent(cloname);
		} catch (Exception e) {
	...
	private void setClassPath() {
		...
		if (container instanceof StandardContext) {
			...
			if (baseClasspath != null) {
				servletContext.setAttribute(Globals.getClassPathAttr(),
						baseClasspath);
				return;
			}
		}
	...
	private String getClasspath(ClassLoader loader) {
		try {
			...
			Throwable t = ExceptionUtils2.unwrapInvocationTargetException(ex);
			ExceptionUtils2.handleThrowable(t);
			if (log.isDebugEnabled())
