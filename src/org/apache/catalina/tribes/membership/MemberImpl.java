...
public class MemberImpl implements Member, java.io.Externalizable {
	 ...
	 * is false
	 */
	private static final boolean DO_DNS_LOOKUPS = Boolean.parseBoolean(System
	 ...
	 */
	@Deprecated
	private static final transient String TCP_LISTEN_PORT = "tcpListenPort";
	/**
	 * @deprecated Unused - will be removed in Tomcat 8.0.x
	 */
	@Deprecated
	private static final transient String TCP_LISTEN_HOST = "tcpListenHost";
	/**
	 * @deprecated Unused - will be removed in Tomcat 8.0.x
	 */
	@Deprecated
	private static final transient String MEMBER_NAME = "memberName";

	private static final transient byte[] TRIBES_MBR_BEGIN = new byte[] { 84,
			82, 73, 66, 69, 83, 45, 66, 1, 0 };
	private static final transient byte[] TRIBES_MBR_END = new byte[] { 84, 82,
	 ...
	 */
	private long memberAliveTime = 0;
	 ...
	 */
	private transient byte[] dataPkg = null;
	 ...
	 */
	private byte[] domain = new byte[0];
	...
	public MemberImpl(String host, int port, long aliveTime) throws IOException {
		...
		this.setMemberAliveTimeData(aliveTime);
	}
	 ...
	 */
	protected void inc() {
		setMsgCountData(getMsgCountData() + 1);
	}
	...
	public int getDataLength() {
		return TRIBES_MBR_BEGIN.length + // start pkg
				...
				4 + // udp port
				1 + // host length
				getHostData().length + // host
				4 + // command length
				getCommandData().length + // command
				4 + // domain length
				getDomainData().length + // domain
				16 + // unique id
				4 + // payload length
				getPayloadData().length + // payload
	...
	public byte[] getData(boolean getalive, boolean reset) {
		if (reset)
			setDataPkgData(null);
		// look in cache first
		if (getDataPkgData() != null) {
			if (getalive) {
				// you'd be surprised, but System.currentTimeMillis
				// shows up on the profiler
				long alive = System.currentTimeMillis() - getServiceStartTime();
				XByteBuffer
						.toBytes(alive, getDataPkgData(), TRIBES_MBR_BEGIN.length + 4);
			}
			return getDataPkgData();
		}
		...
		pos += 8;
		// port
		XByteBuffer.toBytes(getPortData(), data, pos);
		pos += 4;
		// secure port
		XByteBuffer.toBytes(getSecurePortData(), data, pos);
		pos += 4;
		// udp port
		XByteBuffer.toBytes(getUdpPortData(), data, pos);
		pos += 4;
		// host length
		data[pos++] = hl;
		// host
		...
		pos += addr.length;
		// clen - 4 bytes
		XByteBuffer.toBytes(getCommandData().length, data, pos);
		pos += 4;
		// command - clen bytes
		System.arraycopy(getCommandData(), 0, data, pos, getCommandData().length);
		pos += getCommandData().length;
		// dlen - 4 bytes
		XByteBuffer.toBytes(getDomainData().length, data, pos);
		pos += 4;
		// domain - dlen bytes
		System.arraycopy(getDomainData(), 0, data, pos, getDomainData().length);
		pos += getDomainData().length;
		// unique Id
		System.arraycopy(getUniqueIdData(), 0, data, pos, getUniqueIdData().length);
		pos += getUniqueIdData().length;
		// payload
		XByteBuffer.toBytes(getPayloadData().length, data, pos);
		pos += 4;
		System.arraycopy(getPayloadData(), 0, data, pos, getPayloadData().length);
		pos += getPayloadData().length;
	...
	public static MemberImpl getMember(byte[] data, int offset, int length,
		...
		member.setPayloadData(payload);
		member.setDomainData(domain);
		member.setCommandData(command);

		member.setDataPkgData(new byte[length]);
		System.arraycopy(data, offset, member.getDataPkgData(), 0, length);
	...
	public String getHostname() {
		if (this.getHostnameData() != null)
			return getHostnameData();
		else {
			try {
				if (DO_DNS_LOOKUPS)
					this.setHostnameData(java.net.InetAddress.getByAddress(getHostData())
							.getHostName());
				else
					this.setHostnameData(Arrays
							.toString(getHostData(), 0, getHostData().length, true));
				return this.getHostnameData();
			} catch (IOException x) {
	...
	public long getMemberAliveTime() {
		return getMemberAliveTimeData();
	}

	public long getServiceStartTime() {
		return getServiceStartTimeData();
	}
	...
	public String toString() {
		...
		buf.append(getPortData()).append(", alive=");
		buf.append(getMemberAliveTimeData()).append(", ");
		buf.append("securePort=").append(getSecurePortData()).append(", ");
		buf.append("UDP Port=").append(getUdpPortData()).append(", ");
		buf.append("id=").append(bToS(this.getUniqueIdData())).append(", ");
		buf.append("payload=").append(bToS(this.getPayloadData(), 8)).append(", ");
		buf.append("command=").append(bToS(this.getCommandData(), 8)).append(", ");
		buf.append("domain=").append(bToS(this.getDomainData(), 8)).append(", ");
		buf.append("]");
		return buf.toString();
	}
	...
	public boolean equals(Object o) {
		if (o instanceof MemberImpl) {
			return java.util.Arrays.equals(this.getHost(), ((MemberImpl) o).getHost())
					&& this.getPort() == ((MemberImpl) o).getPort()
					&& java.util.Arrays.equals(this.getUniqueId(),
	...
	public void setHostname(String host) throws IOException {
		setHostnameData(host);
		this.setHostData(java.net.InetAddress.getByName(host).getAddress());
	}

	public void setMsgCount(int msgCount) {
		this.setMsgCountData(msgCount);
	}

	public void setPort(int port) {
		this.setPortData(port);
		this.setDataPkgData(null);
	}

	public void setServiceStartTime(long serviceStartTime) {
		this.setServiceStartTimeData(serviceStartTime);
	}

	public void setUniqueId(byte[] uniqueId) {
		this.setUniqueIdData(uniqueId != null ? uniqueId : new byte[16]);
		getData(true, true);
	}

	public void setPayload(byte[] payload) {
		byte[] oldpayload = this.getPayloadData();
		this.setPayloadData(payload != null ? payload : new byte[0]);
		if (this.getData(true, true).length > McastServiceImpl
				.getMAX_PACKET_SIZE()) {
			this.setPayloadData(oldpayload);
			throw new IllegalArgumentException(
	...
	public void setCommand(byte[] command) {
		this.setCommandData(command != null ? command : new byte[0]);
		getData(true, true);
	}

	public void setDomain(byte[] domain) {
		this.setDomainData(domain != null ? domain : new byte[0]);
		getData(true, true);
	}

	public void setSecurePort(int securePort) {
		this.setSecurePortData(securePort);
		this.setDataPkgData(null);
	}

	public void setUdpPort(int port) {
		this.setUdpPortData(port);
		this.setDataPkgData(null);
	}
	...
	public static byte[] getTribesMbrBegin() {
		return TRIBES_MBR_BEGIN;
	}
	...
	public void setHostData(byte[] host) {
		this.host = host;
	}
	...
	public void setHostnameData(String hostname) {
	...
	public int getSecurePortData() {
		return securePort;
	}

	public void setSecurePortData(int securePort) {
		this.securePort = securePort;
	}

	public int getMsgCountData() {
		return msgCount;
	}

	public void setMsgCountData(int msgCount) {
		this.msgCount = msgCount;
	}

	public long getMemberAliveTimeData() {
		return memberAliveTime;
	}

	public void setMemberAliveTimeData(long memberAliveTime) {
		this.memberAliveTime = memberAliveTime;
	}

	public long getServiceStartTimeData() {
		return serviceStartTime;
	}

	public void setServiceStartTimeData(long serviceStartTime) {
		this.serviceStartTime = serviceStartTime;
	}

	public byte[] getDataPkgData() {
		return dataPkg;
	}

	public void setDataPkgData(byte[] dataPkg) {
	...
	public void setCommandData(byte[] command) {
	...
	public void setDomainData(byte[] domain) {
		this.domain = domain;
	}
