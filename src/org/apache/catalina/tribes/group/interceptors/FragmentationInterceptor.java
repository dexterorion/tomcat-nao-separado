...
public class FragmentationInterceptor extends ChannelInterceptorBase {
    ...
    private HashMap<FragmentationInterceptorFragKey, FragmentationInterceptorFragCollection> fragpieces = new HashMap<FragmentationInterceptorFragKey, FragmentationInterceptorFragCollection>();
    ...
    public FragmentationInterceptorFragCollection getFragCollection(FragmentationInterceptorFragKey key, ChannelMessage msg) {
    	FragmentationInterceptorFragCollection coll = fragpieces.get(key);
        if ( coll == null ) {
            synchronized (fragpieces) {
                coll = fragpieces.get(key);
                if ( coll == null ) {
                    coll = new FragmentationInterceptorFragCollection(msg);
    ...
    public void removeFragCollection(FragmentationInterceptorFragKey key) {
        fragpieces.remove(key);
    }
    
    public void defrag(ChannelMessage msg ) { 
    	FragmentationInterceptorFragKey key = new FragmentationInterceptorFragKey(msg.getUniqueId());
        FragmentationInterceptorFragCollection coll = getFragCollection(key,msg);
    ...
    public void heartbeat() {
        try {
            Set<FragmentationInterceptorFragKey> set = fragpieces.keySet(); 
            Object[] keys = set.toArray();
            for ( int i=0; i<keys.length; i++ ) {
            	FragmentationInterceptorFragKey key = (FragmentationInterceptorFragKey)keys[i];
