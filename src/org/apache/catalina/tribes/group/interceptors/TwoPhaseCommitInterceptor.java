...
public class TwoPhaseCommitInterceptor extends ChannelInterceptorBase {
    ...
    private HashMap<UniqueId, TwoPhaseCommitInterceptorMapEntry> messages = new HashMap<UniqueId, TwoPhaseCommitInterceptorMapEntry>();
    ...
    public void messageReceived(ChannelMessage msg) {
        if (okToProcess(msg.getOptions())) {
            if ( msg.getMessage().getLength() == (START_DATA.length+msg.getUniqueId().length+END_DATA.length) &&
                ...
                TwoPhaseCommitInterceptorMapEntry original = messages.get(id);
                if ( original != null ) {
                    super.messageReceived(original.getMsg());
                    messages.remove(id);
                ...
                TwoPhaseCommitInterceptorMapEntry entry = new TwoPhaseCommitInterceptorMapEntry((ChannelMessage)msg.deepclone(),id,System.currentTimeMillis());
    ...
    public void heartbeat() {
        try {
            ...
            Map.Entry<UniqueId,TwoPhaseCommitInterceptorMapEntry>[] entries = messages.entrySet().toArray(new Map.Entry[messages.size()]);
            for (int i=0; i<entries.length; i++ ) {
            	TwoPhaseCommitInterceptorMapEntry entry = entries[i].getValue();
                if ( entry.expired(now,expire) ) {
                    if(log.isInfoEnabled())
                        log.info("Message ["+entry.getId()+"] has expired. Removing.");
                    messages.remove(entry.getId());
                }//end if
            }
        } catch ( Exception x ) {
