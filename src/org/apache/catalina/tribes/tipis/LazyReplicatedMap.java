...
//------------------------------------------------------------------------------   
     ...
     */
    public LazyReplicatedMap(AbstractReplicatedMapMapOwner owner, Channel channel, long timeout, String mapContextName, int initialCapacity, float loadFactor, ClassLoader[] cls) {
     ...
     * @param initialCapacity int - the size of this map, see HashMap
     */
    public LazyReplicatedMap(AbstractReplicatedMapMapOwner owner, Channel channel, long timeout, String mapContextName, int initialCapacity, ClassLoader[] cls) {
        super(owner, channel,timeout,mapContextName,initialCapacity, AbstractReplicatedMap.getDefaultLoadFactor(), Channel.SEND_OPTIONS_DEFAULT, cls, true);
    }
     ...
     * @param mapContextName String - unique name for this map, to allow multiple maps per channel
     */
    public LazyReplicatedMap(AbstractReplicatedMapMapOwner owner, Channel channel, long timeout, String mapContextName, ClassLoader[] cls) {
        super(owner, channel,timeout,mapContextName, AbstractReplicatedMap.getDefaultInitialCapacity(),AbstractReplicatedMap.getDefaultLoadFactor(),Channel.SEND_OPTIONS_DEFAULT, cls, true);
    }
     ...
     */
    public LazyReplicatedMap(AbstractReplicatedMapMapOwner owner, Channel channel, long timeout, String mapContextName, ClassLoader[] cls, boolean terminate) {
        super(owner, channel,timeout,mapContextName, AbstractReplicatedMap.getDefaultInitialCapacity(),
                AbstractReplicatedMap.getDefaultLoadFactor(),Channel.SEND_OPTIONS_DEFAULT, cls, terminate);
    }
    ...
    protected Member[] publishEntryInfo(Object key, Object value) throws ChannelException {
        ...
        do {
            ...
            try {
                ...
                msg = new AbstractReplicatedMapMapMessage(getMapContextName(), AbstractReplicatedMapMapMessage.getMsgBackup(), false,
                                     (Serializable) key, (Serializable) value, null, getChannel().getLocalMember(false), backup);
            ...
            try {
                ...
                if (success && proxies.length > 0 ) {
                    msg = new AbstractReplicatedMapMapMessage(getMapContextName(), AbstractReplicatedMapMapMessage.getMsgProxy(), false,
                                         (Serializable) key, null, null, getChannel().getLocalMember(false),backup);
                    if ( log.isTraceEnabled() ) 
