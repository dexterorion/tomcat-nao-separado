...
public class SetAllPropertiesRule extends Rule {
    ...
    // ----------------------------------------------------- Instance Variables
    private HashMap<String,String> excludes = new HashMap<String,String>();
    ...
    public void begin(String namespace, String nameX, Attributes attributes)
        ...
        for (int i = 0; i < attributes.getLength(); i++) {
            ...
            if ( !excludes.containsKey(name)) {
                if (!getDigester().isFakeAttribute(getDigester().peek(), name) 
                        && !IntrospectionUtils.setProperty(getDigester().peek(), name, value) 
                        && getDigester().getRulesValidation()) {
                    getDigester().getLogger().warn("[SetAllPropertiesRule]{" + getDigester().getMatch() +
