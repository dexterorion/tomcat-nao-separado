...
import org.apache.catalina.realm.RealmBase1;
...
public class Tomcat {
	...
	private Connector connector; // for more - customize the classes
	...
	public static Wrapper addServlet(Context ctx, String servletName,
			Servlet servlet) {
		// will do class for name and set init params
		Wrapper sw = new TomcatExistingStandardWrapper(servlet);
		sw.setName(servletName);
		ctx.addChild(sw);
	...
	public Context addContext(Host host, String contextPath,
		...
		ctx.setDocBase(dir);
		ctx.addLifecycleListener(new TomcatFixContextListener());
	...
	public Context addWebapp(Host host, String url, String name, String path) {
		...
		ctx.addLifecycleListener(new TomcatDefaultWebXmlListener());
	...
	public LifecycleListener getDefaultWebXmlListener() {
		return new TomcatDefaultWebXmlListener();
	}
	...
	public String noDefaultWebXmlPath() {
		return Constants17.getNodefaultwebxml();
	}
	...
	protected void initSimpleAuth() {
		defaultRealm = new RealmBase1(userPass, userRoles, userPrincipals);
	}

	protected void initBaseDir() {
		String catalinaHome = System.getProperty(Globals.getCatalinaHomeProp());
		if (basedir == null) {
			basedir = System.getProperty(Globals.getCatalinaBaseProp());
		}
		if (basedir == null) {
		...
		if (catalinaHome == null) {
			System.setProperty(Globals.getCatalinaHomeProp(), basedir);
		}
		System.setProperty(Globals.getCatalinaBaseProp(), basedir);
	}
	...
	private URL getWebappConfigFileFromJar(File docBase, String url) {
		...
		try {
			...
			JarEntry entry = jar.getJarEntry(Constants17
					.getApplicationcontextxml());
			if (entry != null) {
