...
public final class UserConfig implements LifecycleListener {
	...
	private void deploy() {
		...
		UserDatabase2 database = null;
		try {
			Class<?> clazz = Class.forName(userClass);
			database = (UserDatabase2) clazz.newInstance();
		...
		while (users.hasMoreElements()) {
			...
			results.add(executor.submit(new UserConfigDeployUserDirectory(this, user,
					home)));
		}
	 ...
	 */
	void deploy(String user, String home) {
