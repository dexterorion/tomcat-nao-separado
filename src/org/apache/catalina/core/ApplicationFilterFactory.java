...
public final class ApplicationFilterFactory {
    ...
    public ApplicationFilterFactory() {
    ...
    public ApplicationFilterChain createFilterChain
        ...
        if (request.getAttribute(Globals.getDispatcherTypeAttr()) != null) {
    ...
    private boolean matchDispatcher(FilterMap filterMap, DispatcherType type) {
        switch (type) {
            case FORWARD : {
                if ((filterMap.getDispatcherMapping() & FilterMap.getForward()) > 0) {
                        return true;
                }
                break;
            }
            case INCLUDE : {
                if ((filterMap.getDispatcherMapping() & FilterMap.getInclude()) > 0) {
                    return true;
                }
                break;
            }
            case REQUEST : {
                if ((filterMap.getDispatcherMapping() & FilterMap.getRequest()) > 0) {
                    return true;
                }
                break;
            }
            case ERROR : {
                if ((filterMap.getDispatcherMapping() & FilterMap.getError()) > 0) {
                    return true;
                }
                break;
            }
            case ASYNC : {
                if ((filterMap.getDispatcherMapping() & FilterMap.getAsync()) > 0) {
                    return true;
                }
                break;
            }
