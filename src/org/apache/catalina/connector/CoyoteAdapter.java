...
import org.apache.catalina.util.SessionConfig2;
...
public class CoyoteAdapter implements Adapter {
	...
	private static final int ADAPTER_NOTES = 1;

	private static final boolean ALLOW_BACKSLASH = Boolean
			.valueOf(
					System.getProperty(
	 ...
	 */
	@Override
	public boolean event(Request2 req,
			Response3 res, SocketStatus status) {
		...
		try {
			if (status == SocketStatus.OPEN_READ) {
				if (response.isClosed()) {
					...
					request.getEvent().setEventType(CometEventEventType.END);
					...
					if (read) {
						request.getEvent().setEventType(
								CometEventEventType.READ);
						request.getEvent().setEventSubType(null);
					} else if (error) {
						request.getEvent().setEventType(
								CometEventEventType.ERROR);
						request.getEvent().setEventSubType(
								CometEventEventSubType.CLIENT_DISCONNECT);
					} else {
						request.getEvent().setEventType(
								CometEventEventType.END);
						request.getEvent().setEventSubType(null);
					}
				}
			} else if (status == SocketStatus.DISCONNECT) {
				request.getEvent().setEventType(CometEventEventType.ERROR);
				request.getEvent().setEventSubType(
						CometEventEventSubType.CLIENT_DISCONNECT);
				error = true;
				...
				request.getEvent().setEventType(CometEventEventType.ERROR);
				request.getEvent().setEventSubType(
						CometEventEventSubType.IOEXCEPTION);
				error = true;
			} else if (status == SocketStatus.STOP) {
				request.getEvent().setEventType(CometEventEventType.END);
				request.getEvent().setEventSubType(
						CometEventEventSubType.SERVER_SHUTDOWN);
				...
				if (response.isClosed()) {
					...
					request.getEvent().setEventType(CometEventEventType.ERROR);
					request.getEvent().setEventSubType(
							CometEventEventSubType.TIMEOUT);
				}
			}
			...
			if (response.isClosed() || !request.isComet()) {
				if (status == SocketStatus.OPEN_READ
						&& request.getEvent().getEventType() != CometEventEventType.END) {
			...
			if (error || response.isClosed() || !request.isComet()) {
				((Context) request.getMappingData().getContext()).logAccess(
						request, response,
	...
	@Override
	public boolean asyncDispatch(Request2 req,
			Response3 res, SocketStatus status)
			throws Exception {
		Request request = (Request) req.getNote(ADAPTER_NOTES);
		Response response = (Response) res.getNote(ADAPTER_NOTES);
		...
		try {
			if (!request.isAsync() && !comet) {
				...
				// this state
				Context ctxt = (Context) request.getMappingData().getContext();
				if (ctxt != null) {
	 ...
	 */
	@Override
	public void service(Request2 req,
			Response3 res) throws Exception {
		...
		try {
			...
			if (asyncConImpl != null) {
				...
				response.finishResponse();
				if (postParseSuccess
						&& request.getMappingData().getContext() != null) {
					// Log only if processing was invoked.
					// If postParseRequest() failed, it has already logged it.
					// If context is null this was the start of a comet request
					// that failed and has already been logged.
					((Context) request.getMappingData().getContext())
							.logAccess(
									request,
									response,
									System.currentTimeMillis()
											- req.getStartTime(), false);
				}
				req.action(ActionCode.POST_REQUEST, null);
			}
	...
	@Override
	public void errorDispatch(Request2 req,
			Response3 res) {
		Request request = (Request) req.getNote(ADAPTER_NOTES);
		Response response = (Response) res.getNote(ADAPTER_NOTES);

		if (request != null && request.getMappingData().getContext() != null) {
			((Context) request.getMappingData().getContext()).logAccess(
					request, response,
					System.currentTimeMillis() - req.getStartTime(), false);
		} else {
	...
	@Override
	public void log(Request2 req,
			Response3 res, long time) {
		...
		try {
			...
			if (request.getMappingData() != null) {
				if (request.getMappingData().getContext() != null) {
					logged = true;
					((Context) request.getMappingData().getContext()).logAccess(
							request, response, time, true);
				} else if (request.getMappingData().getHost() != null) {
					logged = true;
					((Host) request.getMappingData().getHost()).logAccess(request,
							response, time, true);
				}
			}
			if (!logged) {
	...
	@Override
	public void checkRecycled(Request2 req,
			Response3 res) {
		Request request = (Request) req.getNote(ADAPTER_NOTES);
		Response response = (Response) res.getNote(ADAPTER_NOTES);
			...
			if (connector.getState().isAvailable()) {
				if (log.isInfoEnabled()) {
					log.info(sm.getString(messageKey),
							new CoyoteAdapterRecycleRequiredException());
				}
			} else {
	 ...
	 */
	protected boolean postParseRequest(Request2 req,
			Request request, Response3 res, Response response)
			throws Exception {
		...
		while (mapRequired) {
			...
			if (request.getServletContext().getEffectiveSessionTrackingModes()
				...
				sessionID = request.getPathParameter(SessionConfig2
			...
			if (version != null && request.getContext() == versionContext) {
				...
				Object[] contexts = request.getMappingData().getContexts();
				...
				if (contexts != null && sessionID != null) {
					...
					for (int i = (contexts.length); i > 0; i--) {
						...
						if (ctxt.getManager().findSession(sessionID) != null) {
							...
							// already been mapped?
							if (!ctxt.equals(request.getMappingData()
									.getContext())) {
		...
		// Possible redirect
		MessageBytes redirectPathMB = request.getMappingData()
				.getRedirectPath();
		if (!redirectPathMB.isNull()) {
			...
			if (request.isRequestedSessionIdFromURL()) {
				...
				redirectPath = redirectPath
						+ ";"
						+ SessionConfig2.getSessionUriParamName(request
								.getContext()) + "="
	 ...
	 * @param request
	 */
	protected void parsePathParameters(Request2 req,
			Request request) {
	...
	protected void parseSessionSslId(Request request) {
		if (request.getRequestedSessionId() == null
				...
				&& request.getConnector().getSecure()) {
	 ...
	 */
	protected void parseSessionCookiesId(Request2 req,
			Request request) {
		...
		String sessionCookieName = SessionConfig2.getSessionCookieName(context);
	...
	public static void setUrlEncoder(URLEncoder urlEncoder) {
		CoyoteAdapter.urlEncoder = urlEncoder;
	}
	...
	public static EnumSet<SessionTrackingMode> getSslOnly() {
		return SSL_ONLY;
	}

	public static int getAdapterNotes() {
		return ADAPTER_NOTES;
	}
