...
public class ManagerServlet extends HttpServlet implements ContainerServlet {
	...
	private static final boolean LAST_ACCESS_AT_START;
	...
	static {
		...
		if (lastAccessAtStart == null) {
			LAST_ACCESS_AT_START = Globals.isStrictServletCompliance();
		} else {
	 ...
	 */
	private File deployed = null;

	/**
	 * Path used to store revisions of webapps.
	 */
	private File versioned = null;
	 ...
	 */
	@Deprecated
	private File contextDescriptors = null;
	 ...
	 */
	private transient javax.naming.Context global = null;

	/**
	 * The string manager for this package.
	 */
	private static final StringManager3 sm = StringManager3.getManager(Constants10
			.getPackage());
	...
	public void setWrapper(Wrapper wrapper) {
		...
		mBeanServer = Registry2.getRegistry(null, null).getMBeanServer();
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		response.setContentType("text/plain; charset=" + Constants10.getCharset());
	...
	public void init() throws ServletException {
		...
		if (!deployed.isAbsolute()) {
			deployed = new File(System.getProperty(Globals
					.getCatalinaBaseProp()), appBase);
		}
		configBase = new File(
				System.getProperty(Globals.getCatalinaBaseProp()), "conf");
	...
	protected synchronized void save(PrintWriter writer, String path,
		...
		if ((path == null) || path.length() == 0 || !path.startsWith("/")) {
			...
			javax.naming.Context context = (javax.naming.Context) host.findChild(contextPath);
			if (context == null) {
	...
	protected void deploy(PrintWriter writer, String config, ContextName cn,
			String war, boolean update, StringManager3 smClient) {
	...
	private void writeDeployResult(PrintWriter writer, StringManager3 smClient,
	 ...
	 *            Writer to render to
	 */
	protected void list(PrintWriter writer, StringManager3 smClient) {
	 ...
	 *            Writer to render to
	 */
	protected void serverinfo(PrintWriter writer, StringManager3 smClient) {
		if (debug >= 1)
			log("serverinfo");
		try {
		...
		try {
			...
			writer.println(smClient.getString(
					"managerServlet.sessiondefaultmax", ""
							+ maxInactiveInterval));
			Session2[] sessions = manager.findSessions();
	 ...
	 *
	 * @deprecated Use
	 *             {@link #sessions(PrintWriter, ContextName, int, StringManager3)}
	 */
	@Deprecated
	...
	protected void expireSessions(PrintWriter writer, ContextName cn,
			HttpServletRequest req, StringManager3 smClient) {
		int idle = -1;
	...
	protected void uploadWar(PrintWriter writer, HttpServletRequest request,
			File war, StringManager3 smClient) throws IOException {
	...
	/**
	 * @deprecated Use {@link StringManager3#getManager(String, Enumeration)}.
	 *             This method will be removed in Tomcat 8.
	 */
	@Deprecated
	protected StringManager3 getStringManager(HttpServletRequest req) {
		Enumeration<Locale> requestedLocales = req.getLocales();
		while (requestedLocales.hasMoreElements()) {
			Locale locale = requestedLocales.nextElement();
			StringManager3 result = StringManager3.getManager(
					Constants10.getPackage(), locale);
	...
	protected static boolean validateContextName(ContextName cn,
			PrintWriter writer, StringManager3 sm) {
	...
	public void setDeployed(File deployed) {
		this.deployed = deployed;
	}

	public File getVersioned() {
		return versioned;
	}

	public void setVersioned(File versioned) {
		this.versioned = versioned;
	}

	public File getContextDescriptors() {
		return contextDescriptors;
	}

	public void setContextDescriptors(File contextDescriptors) {
	...
	public javax.naming.Context getGlobal() {
		return global;
	}

	public void setGlobal(javax.naming.Context global) {
		this.global = global;
	}
	...
	public static boolean isLastAccessAtStart() {
		return LAST_ACCESS_AT_START;
	}
	...
	public void setAppBase(File appBase) {
