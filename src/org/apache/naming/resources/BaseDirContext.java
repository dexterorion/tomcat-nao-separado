...
import org.apache.naming.StringManager2;
...
public abstract class BaseDirContext implements DirContext {
	 ...
	 */
	private static final StringManager2 sm = StringManager2
			.getManager(Constants31.getPackage());
	 ...
	 */
	private int cacheObjectMaxSize = 512; // 512 K

	/**
	 * Aliases allow content to be included from other locations.
	 */
	private Map<String, BaseDirContext> aliases = new HashMap<String, BaseDirContext>();
	 ...
	 */
	private List<DirContext> altDirContexts = new ArrayList<DirContext>();
	...
	public String getRealPath(String name) {
		if (!aliases.isEmpty()) {
			BaseDirContextAliasResult result = findAlias(name);
			if (result.getDirContext() != null) {
				return result.getDirContext().doGetRealPath(result.getAliasName());
			}
		}
	...
	private Object doLookupWithoutNNFE(String name) throws NamingException {
		if (!aliases.isEmpty()) {
			BaseDirContextAliasResult result = findAlias(name);
			if (result.getDirContext() != null) {
				return result.getDirContext().lookup(result.getAliasName());
			}
		}
	...
	public NamingEnumeration<NameClassPair> list(String name)
		...
		if (!aliases.isEmpty()) {
			BaseDirContextAliasResult result = findAlias(name);
			if (result.getDirContext() != null) {
				return result.getDirContext().list(result.getAliasName());
			}
		}
	...
	public final NamingEnumeration<Binding> listBindings(String name)
		...
		if (!aliases.isEmpty()) {
			BaseDirContextAliasResult result = findAlias(name);
			if (result.getDirContext() != null) {
				return result.getDirContext().listBindings(result.getAliasName());
			}
		}
	...
	public final Attributes getAttributes(String name, String[] attrIds)
		...
		if (!aliases.isEmpty()) {
			BaseDirContextAliasResult result = findAlias(name);
			if (result.getDirContext() != null) {
				return result.getDirContext().getAttributes(result.getAliasName(),
						attrIds);
			}
		}
	...
	// -------------------------------------------------------- Private Methods
	private BaseDirContextAliasResult findAlias(String name) {
		BaseDirContextAliasResult result = new BaseDirContextAliasResult();
		...
		result.setDirContext(aliases.get(searchName));
		while (result.getDirContext() == null) {
		...
		if (result.getDirContext() != null) {
			if (slashAppended) {
				result.setAliasName(name.substring(searchName.length() - 1));
			} else {
				result.setAliasName(name.substring(searchName.length()));
			}
		}
	...
	public List<DirContext> getAltDirContexts() {
		return altDirContexts;
	}

	public void setAltDirContexts(List<DirContext> altDirContexts) {
		this.altDirContexts = altDirContexts;
	}
	...
	public static StringManager2 getSm() {
		return sm;
	}

	public void setAliases(Map<String, BaseDirContext> aliases) {
