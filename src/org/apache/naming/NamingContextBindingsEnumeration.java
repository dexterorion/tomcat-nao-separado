...
public class NamingContextBindingsEnumeration implements NamingEnumeration<Binding> {
     ...
     */
    private Iterator<NamingEntry> iterator;
    ...
    private Binding nextElementInternal() throws NamingException {
        ...
        if (entry.getType() == NamingEntry.getReference()
                || entry.getType() == NamingEntry.getLinkRef()) {
            try {
                value = ctx.lookup(new CompositeName(entry.getName()));
            } catch (NamingException e) {
                throw e;
            } catch (Exception e) {
        ...
        return new Binding(entry.getName(), value.getClass().getName(), value, true);
    }
}
