...
public class SetPropertiesRule extends Rule {
    ...
    public void begin(String namespace, String theName, Attributes attributes)
        ...
        if (getDigester().getLog().isDebugEnabled()) {
            if (top != null) {
                getDigester().getLog().debug("[SetPropertiesRule]{" + getDigester().getMatch() +
        ...
        for (int i = 0; i < attributes.getLength(); i++) {
            ...
            if (!getDigester().isFakeAttribute(top, name) 
                    && !IntrospectionUtils.setProperty(top, name, value) 
                    && getDigester().getRulesValidation()) {
                getDigester().getLog().warn("[SetPropertiesRule]{" + getDigester().getMatch() +
                        "} Setting property '" + name + "' to '" +
