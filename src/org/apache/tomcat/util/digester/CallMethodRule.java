...
public class CallMethodRule extends Rule {
    ...
    /**
     * Set the associated getDigester().
    ...
    public void setDigester(Digester digester)
    {
        ...
        if (this.paramClassNames != null) {
            ...
            for (int i = 0; i < this.paramClassNames.length; i++) {
                try {
                    this.paramTypes[i] =
                            getDigester().getClassLoader().loadClass(this.paramClassNames[i]);
                } catch (ClassNotFoundException e) {
                    // use the digester log
                    getDigester().getLogger().error("(CallMethodRule) Cannot load class " + this.paramClassNames[i], e);
    ...
    public void end(String namespace, String name) throws Exception {
        ...
        if (paramCount > 0) {
            ...
            if (getDigester().getLog().isTraceEnabled()) {
                for (int i=0,size=parameters.length;i<size;i++) {
                    getDigester().getLog().trace("[CallMethodRule](" + i + ")" + parameters[i]) ;
                }
            }
        ...
        if (getDigester().getLog().isDebugEnabled()) {
            ...
            sb.append(")");
            getDigester().getLog().debug(sb.toString());
        }
