...
public class SecureNioChannel extends NioChannel  {
    ...
    private HandshakeStatus handshakeStatus; //gets set by handshake
...
//===========================================================================================
    ...
    protected SSLEngineResult handshakeUnwrap(boolean doread) throws IOException {
        ...
        if ( doread )  {
            ...
            int read = getSc().read(netInBuffer);
    ...
    public int read(ByteBuffer dst) throws IOException {
        ...
        if ( dst != getBufHandler().getReadBuffer() ) throw new IllegalArgumentException("You can only read using the application read buffer provided by the handler.");
        ...
        int netread = getSc().read(netInBuffer);
    ...
    public int write(ByteBuffer src) throws IOException {
        ...
        if ( src == this.netOutBuffer ) {
            ...
            int written = getSc().write(src);
            return written;
        } else {
            //make sure we can handle expand, and that we only use on buffer
            if ( (!this.isSendFile()) && (src != getBufHandler().getWriteBuffer()) ) throw new IllegalArgumentException("You can only write using the application write buffer provided by the handler.");
