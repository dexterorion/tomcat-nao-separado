...
public class AprServletOutputStream extends AbstractServletOutputStream {
    ...
    protected int doWrite(boolean block, byte[] b, int off, int len)
        ...
        if (closed) {
            throw new IOException(getSm().getString("apr.closed", Long.valueOf(socket)));
        }
    ...
    private int doWriteInternal(byte[] b, int off, int len) throws IOException {
        ...
        do {
            ...
            if (Status.APR_STATUS_IS_EAGAIN(-written)) {
                written = 0;
            } else if (-written == Status.getAprEof()) {
                throw new EOFException(getSm().getString("apr.clientAbort"));
            } else if ((OS.isWin32() || OS.isWin64()) &&
                    (-written == Status.getAprOsStartSyserr() + 10053)) {
                // 10053 on Windows is connection aborted
                throw new EOFException(getSm().getString("apr.clientAbort"));
            } else if (written < 0) {
                throw new IOException(getSm().getString("apr.write.error",
