...
public final class AstValue extends SimpleNode {
    ...
    private static final boolean COERCE_TO_ZERO;
    ...
    public Class<?> getType(EvaluationContext ctx) throws ELException {
    	AstValueTarget t = getTarget(ctx);
        ctx.setPropertyResolved(false);
        Class<?> result = ctx.getELResolver().getType(ctx, t.getBase(), t.getProperty());
        if (!ctx.isPropertyResolved()) {
            throw new PropertyNotFoundException(MessageFactory.get(
                    "error.resolver.unhandled", t.getBase(), t.getProperty()));            
        }
        return result;
    }

    private final AstValueTarget getTarget(EvaluationContext ctx) throws ELException {
        // evaluate expr-a to value-a
        Object base = this.getChildren()[0].getValue(ctx);

        // if our base is null (we know there are more properties to evaluate)
        if (base == null) {
            throw new PropertyNotFoundException(MessageFactory.get(
                    "error.unreachable.base", this.getChildren()[0].getImage()));
        }
        ...
        while (i < propCount) {
            if (i + 2 < propCount &&
                    this.getChildren()[i + 1] instanceof AstMethodParameters) {
                // Method call not at end of expression
                base = resolver.invoke(ctx, base,
                        this.getChildren()[i].getValue(ctx), null,
                        ((AstMethodParameters)
                                this.getChildren()[i + 1]).getParameters(ctx));
                i += 2;
            } else if (i + 2 == propCount &&
                    this.getChildren()[i + 1] instanceof AstMethodParameters) {
                // Method call at end of expression
                ctx.setPropertyResolved(false);
                property = this.getChildren()[i].getValue(ctx);
                i += 2;
        ...
        AstValueTarget t = new AstValueTarget();
        t.setBase(base);
    ...
    public Object getValue(EvaluationContext ctx) throws ELException {
        ...
        while (base != null && i < propCount) {
            suffix = this.getChildren()[i].getValue(ctx);
            if (i + 1 < propCount &&
                    (this.getChildren()[i+1] instanceof AstMethodParameters)) {
                AstMethodParameters mps =
                    (AstMethodParameters) this.getChildren()[i+1];
                // This is a method
    ...
    public boolean isReadOnly(EvaluationContext ctx) throws ELException {
        ...
        ctx.setPropertyResolved(false);
        boolean result =
            ctx.getELResolver().isReadOnly(ctx, t.getBase(), t.getProperty());
        if (!ctx.isPropertyResolved()) {
            throw new PropertyNotFoundException(MessageFactory.get(
                    "error.resolver.unhandled", t.getBase(), t.getProperty()));            
        }
    ...
    public void setValue(EvaluationContext ctx, Object value)
        ...
        Class<?> targetClass = resolver.getType(ctx, t.getBase(), t.getProperty());
        if (COERCE_TO_ZERO == true
                || !isAssignable(value, targetClass)) {
            resolver.setValue(ctx, t.getBase(), t.getProperty(),
                    ELSupport.coerceToType(value, targetClass));
        } else {
            resolver.setValue(ctx, t.getBase(), t.getProperty(), value);
        }
        if (!ctx.isPropertyResolved()) {
            throw new PropertyNotFoundException(MessageFactory.get(
                    "error.resolver.unhandled", t.getBase(), t.getProperty()));            
    ...
    public MethodInfo getMethodInfo(EvaluationContext ctx, 
        ...
        Method m = ReflectionUtil.getMethod(
                t.getBase(), t.getProperty(), paramTypes, null);
    ...
    public Object invoke(EvaluationContext ctx, 
        ...
        m = ReflectionUtil.getMethod(t.getBase(), t.getProperty(), types, values);
        ...
        Object result = null;
        try {
            result = m.invoke(t.getBase(), values);
    ...
    public ValueReference getValueReference(EvaluationContext ctx) {
        ...
        AstValueTarget t = getTarget(ctx);
        return new ValueReference(t.getBase(), t.getProperty());
    }
