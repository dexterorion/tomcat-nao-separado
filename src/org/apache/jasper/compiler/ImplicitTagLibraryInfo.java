...
public class ImplicitTagLibraryInfo extends TagLibraryInfo {
    ...
    public ImplicitTagLibraryInfo(JspCompilationContext ctxt,
        ...
        this.setFunctions(new FunctionInfo[0]);

        setTlibversion(TLIB_VERSION);
        setJspversion(JSP_VERSION);
        ...
        if (tagdir.equals(WEB_INF_TAGS)
            ...
            setShortname(TAGS_SHORTNAME);
        } else {
            setShortname(tagdir.substring(WEB_INF_TAGS.length()));
            setShortname(getShortname().replace('/', '-'));
        }
                ...
                if (path.endsWith(TAG_FILE_SUFFIX)
                    ...
                    try {
                        ...
                        if (in != null) {
                            ...
                            TreeNode tld = pu.parseXMLDocument(getUri(), in);
                            ...
                            try {
                                double version = Double.parseDouble(this.getJspversion());
                                if (version < 2.0) {
    ...
    public TagFileInfo getTagFile(String shortName) {
        ...
        if (tagFile == null) {
            ...
            this.setTagFiles(new TagFileInfo[vec.size()]);
            vec.copyInto(this.getTagFiles());
        }
