...
public class JspReader {
     ...
     */
    public String getFile(final int fileid) {
    ...
    public boolean hasMoreInput() throws JasperException {
        if (current.getCursor() >= current.getStream().length) {
            if (singleFile) return false; 
            while (popFile()) {
                if (current.getCursor() < current.getStream().length) return true;
            }
            return false;
        }
        return true;
    ...
    public int nextChar() throws JasperException {
        ...
        int ch = current.getStream()[current.getCursor()];

        current.setCursor(current.getCursor() + 1);
        
        if (ch == '\n') {
            current.setLine(current.getLine() + 1);
            current.setCol(0);
        } else {
    ...
    private Boolean indexOf(char c, Mark mark) throws JasperException {
        ...
        int end = current.getStream().length;
        int ch;
        int line = current.getLine();
        int col = current.getCol();
        int i = current.getCursor();
        for(; i < end; i ++) {
           ch = current.getStream()[i];
    ...
    public String getText(Mark start, Mark stop) throws JasperException {
        Mark oldstart = mark();
        reset(start);
    ...
    public int peekChar() throws JasperException {
        if (!hasMoreInput())
            return -1;
        return current.getStream()[current.getCursor()];
    }
    ...
    public boolean matches(String string) throws JasperException {
       ...
       int streamSize = current.getStream().length;
     ...
     */
    public Mark skipELExpression() throws JasperException {
        // ELExpressionBody.
    ...
    public final boolean isSpace() throws JasperException {
