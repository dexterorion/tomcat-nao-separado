...
public class JspUtil {
    ...
    public static void checkAttributes(String typeOfTag, Node n,
    		JspUtilValidAttribute[] validAttributes, ErrorDispatcher err)
            throws JasperException {
        ...
        NodeNodes tagBody = n.getBody();
        if (tagBody != null) {
            int numSubElements = tagBody.size();
            for (int i = 0; i < numSubElements; i++) {
                Node node = tagBody.getNode(i);
                if (node instanceof NodeNamedAttribute) {
        ...
        for (int i = 0; i < validAttributes.length; i++) {
            int attrPos;
            if (validAttributes[i].isMandatory()) {
                attrPos = temp.indexOf(validAttributes[i].getName());
                if (attrPos != -1) {
                    temp.remove(attrPos);
                    valid = true;
                } else {
                    valid = false;
                    missingAttribute = validAttributes[i].getName();
                    break;
                }
            }
        }
        ...
        for (int j = 0; j < attrLeftLength; j++) {
            ...
            for (int i = 0; i < validAttributes.length; i++) {
                if (attribute.equals(validAttributes[i].getName())) {
                    valid = true;
                    break;
                }
            }
    ...
    public static String getTagHandlerClassName(String path, String urn,
        ...
        if (index != -1) {
            className = Constants28.getTagFilePackageName() + ".web";
    ...
    private static String getClassNameBase(String urn) {
        StringBuilder base =
                new StringBuilder(Constants28.getTagFilePackageName() + ".meta.");
        if (urn != null) {
    ...
    public static InputStreamReader getReader(String fname, String encoding,
