...
import org.apache.jasper.el.FunctionMapperImpl2;
import org.apache.jasper.el.VariableResolverImpl;
import org.apache.jasper.security.SecurityUtil2;
...
public class PageContextImpl extends PageContext {
    ...
    private void _initialize(Servlet servlet, ServletRequest request,
        ...
        depth = -1;
        if (bufferSize == JspWriter.getDefaultBuffer()) {
            bufferSize = Constants28.getDefaultBufferSize();
        }
        ...
        setAttribute(getOutVariable(), this.out);
        setAttribute(getRequestVariable(), request);
        setAttribute(getResponseVariable(), response);

        if (session != null)
            setAttribute(getSessionVariable(), session);

        setAttribute(getPageVariable(), servlet);
        setAttribute(getConfig(), config);
        setAttribute(getPagecontext(), this);
        setAttribute(getApplication(), context);
    ...
    public Object getAttribute(final String name) {
        ...
        if (SecurityUtil2.isPackageProtectionEnabled()) {
    ...
    private void doRemoveAttribute(String name) {
        removeAttribute(name, getPageScope());
        removeAttribute(name, getRequestScope());
        if( session != null ) {
            try {
                removeAttribute(name, getSessionScope());
            } catch(IllegalStateException ise) {
                // Session has been invalidated.
                // Ignore and fall throw to application scope.
            }
        }
        removeAttribute(name, getApplicationScope());
    }
    ...
    public JspWriter pushBody(Writer writer) {
        ...
        setAttribute(getOutVariable(), out);
    ...
    private void doHandlePageException(Throwable t) throws IOException,
        ...
        if (errorPageURL != null && !errorPageURL.equals("")) {
             ...
             */
            request.setAttribute(PageContext.getExceptionVariable(), t);
            ...
            request.removeAttribute(PageContext.getExceptionVariable());
            ...
            } else if (t instanceof ELException2) {
                rootCause = ((ELException2) t).getCause();
            }
    ...
    public static Object proprietaryEvaluate(final String expression,
        ...
        if (SecurityUtil2.isPackageProtectionEnabled()) {
            try {
                retValue = AccessController
                        .doPrivileged(new PrivilegedExceptionAction<Object>() {
                            ...
                            public Object run() throws Exception {
                                ...
                                ctx.setFunctionMapper(new FunctionMapperImpl2(functionMap));
                ...
                if (realEx instanceof ELException2) {
                    throw (ELException2) realEx;
                } else {
                    throw new ELException2(realEx);
                }
            }
        } else {
            ELContextImpl ctx = (ELContextImpl) pageContext.getELContext();
