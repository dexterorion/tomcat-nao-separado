...
public class FastRemovalDequeue<T> {
    ...
    private FastRemovalDequeueEntry<T> first;
    /** Last element of the queue. */
    private FastRemovalDequeueEntry<T> last;
    /** Size of the queue */
    private int size;
     ...
     * */
    public synchronized FastRemovalDequeueEntry<T> push(final T object) {
    	FastRemovalDequeueEntry<T> entry = new FastRemovalDequeueEntry<T>(object);
        if (size >= maxSize) {
     ...
     * @return an entry for use when the object should be moved.
     * */
    public synchronized FastRemovalDequeueEntry<T> unpop(final T object) {
    	FastRemovalDequeueEntry<T> entry = new FastRemovalDequeueEntry<T>(object);
        if (size >= maxSize) {
    ...
    public synchronized T unpush() {
        T content = null;
        if (first != null) {
        	FastRemovalDequeueEntry<T> element = first;
            first = first.getNext();
    ...
    public synchronized T pop() {
        T content = null;
        if (last != null) {
        	FastRemovalDequeueEntry<T> element = last;
     ...
     **/
    public synchronized void remove(final FastRemovalDequeueEntry<T> element) {
        if (element == null || !element.getValid()) {
            return;
        }
        FastRemovalDequeueEntry<T> next = element.getNext();
        FastRemovalDequeueEntry<T> prev = element.getPrevious();
        if (next != null) {
     ...
     * */
    public synchronized void moveFirst(final FastRemovalDequeueEntry<T> element) {
        if (element.getValid() &&
     ...
     * */
    public synchronized void moveLast(final FastRemovalDequeueEntry<T> element) {
        if (element.getValid() &&
